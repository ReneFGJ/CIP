...
$sql = "delete from pibic_semic_avaliador where psa_p05 = 'SEMIC19'";
$rlt = db_query($sql);
...
///////////////////////////////////////////
$sql .= "order by us_nome, abbrev, article_seq, psa_p03 ";
//$sql .= " limit 30 ";
$rlt = db_query($sql);
$tot = 0;
$idx = "X";
echo '<H1>Avaliadores x Trabalhos</H1>';
...
while ($line = db_read($rlt))
	...
	if ($avaliador != $idx)
		{
		echo '<TR>';
		echo '<TD colspan="5"><B>'.$avaliador.'</TD>';
		$idx = $avaliador;
		}
	echo '<TR '.coluna().'>';
	echo '<TD>'.$cor.$idart.'</TD>';
	echo '<TD>'.$cor.$tipoe.'</TD>';
	echo '<TD>'.$cor.$tipo.'</TD>';
	echo '<TD>'.$cor.substr($titulo,0,50).'</TD>';
	echo '<TD>'.$cor.$id.'</TD>';
