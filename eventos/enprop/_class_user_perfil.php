...
class user_perfil
	...
	function ghost($id)
		{
			global $http,$secu;
			$link = $http.'user_ghost_mode.php?dd0='.$id.'&dd90='.checkpost($id.$secu);
	...
	function mostrar($id)
		{
			$sql = "select * from ".$this->tabela_usuario." where us_cracha = '".$id."' ";
			$rlt = db_query($sql);
			$sx = '<table class="tabela00" width="100%">';
			if ($line = db_read($rlt))
				{
					$sx .= '<TR><TD align="left" width="10%">Usuário:';
					$sx .= '<TD class="tabela01"><B>'.$line['us_login'].'</B>';
					$sx .= '<TR><TD align="left">Crachá:';
					$sx .= '<TD class="tabela01"><B>'.$line['us_cracha'].'</B>';
					$sx .= '<TR><TD align="left">Nome:';
					$sx .= '<TD class="tabela01"><B>'.$line['us_nome'].'</B>';
					$sx .= '<TR><TD align="left">e-mail:';
					$sx .= '<TD class="tabela01"><B>'.$line['us_email'].'</B>';
				}
			$sx .= '</table>';
			return($sx);
		}
