...
$sql = "select * from articles where id_article = '" . $dd[0] . "' ";
$rlt = db_query($sql);
$line = db_read($rlt);
$trabalho = trim($line['article_title']);
$trabalho = troca($trabalho, chr(13), ' ');
$trabalho = troca($trabalho, chr(10), '');
$trabalho = troca($trabalho, '  ', ' ');
$aluno = trim($line['article_autores']);
$al = $line['article_author'];
$al = troca($al, chr(13), '#');
$aln = splitx('#', $al);
$participantes = '';
$aluno = $aln[0];
if (strpos($aluno, ';') > 0) {
	$aluno = substr($aluno, 0, strpos($aluno, ';'));
}
$pa = 0;
for ($r = 1; $r < count($aln) - 1; $r++) {
	if (strlen($aln[$r]) > 0) {
		$autor = substr($aln[$r], 0, strpos($aln[$r], ';'));
	} else {
		$autor = $aln[$r];
	}
	if (strlen($autor) > 0) {
		if (strpos($autor, ',') > 0) {
			$autor = substr($autor, strpos($autor, ',') + 1, strlen($autor)) . ' ' . substr($autor, 0, strpos($autor, ','));
		}
		$autor = nbr_autor(lowercase($autor), 4);
