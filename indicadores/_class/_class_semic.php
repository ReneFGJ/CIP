...
class semic
	...
	function inport_file_cadastro($txt)
		{
			//$this->structure();
			...
			$xnome = '';
			$isql = '';
			$sql = "delete from semic_ouvinte_cadastro where 1=1 ";
			$rlt = db_query($sql);
			...
			for ($r=1;$r < count($ln);$r++)
				{
					...
					$sm_cpf 	= strzero(sonumero(trim($nn[2])),11);
					$sm_nome 	= troca(trim($nn[0]),"'",'´');
					$sm_inst 	= troca(trim($nn[1]),"'",'´');					

							echo '<HR>';
							echo '<BR>Nome: '.$sm_nome;
							echo '<BR>CPF: '.$sm_cpf;
							echo '<BR>Instituição: '.$sm_inst;
							echo '<HR>';
						$isql = "insert into semic_ouvinte_cadastro
									(
									sc_nome, sc_cpf, sc_instituicao
									) values (
									'$sm_nome','$sm_cpf','$sm_inst'
									); ";	
						$rlt = db_query($isql);
				}
	...
	function structure()
		{
			...
			$sql = "
			CREATE TABLE semic_ouvinte_cadastro
			(
				id_sc serial not null,
				sc_nome char(100),
				sc_cpf char(15),
				sc_instituicao char(70)
			)
			";
			$rlt = db_query($sql);
