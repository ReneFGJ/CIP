			...
			while ($line = db_read($rlt))
				{
					...
					if ($status == '@') { $cor = '<font color="green">'; $status = 'Em submissão'; }
					if ($status == 'B') { $cor = '<font color="black">'; $status = 'Concluído'; }
					if ($status == 'C') { $cor = '<font color="black">'; $status = 'Aceito'; }
					if ($status == 'D') { $cor = '<font color="black">'; $status = 'Finalizado'; }
					if ($status == 'P') { $cor = '<font color="red">'; $status = 'Problemas Coordenação'; }
	...
	function projetos_area($area='',$ano='2013')
		{
			...
			if ($line = db_read($rlt))
				{
					echo $this->projeto_mostra_mini($line);
					exit;
				}
			return($sx);
		}
			...
			if ((strlen($acao) > 0) and (strlen($dd[3]) > 0))
				{
					...
					$ac = $dd[3];
					$std = $dd[3];
					if ($std == '@') { $std = 'Devolvido ao professor para correções '; $ac = 1; }
					if ($std == 'A') { $std = 'Aceito submissão manualmente '; $ac = 2; }					
					if ($std == 'C') { $std = 'Aceito para avaliação '; $ac = 6; }
					if ($std == 'T') { $std = 'Análise da TI '; $ac = 7; }
					if ($std == 'P') { $std = 'Análise da Coordenação '; $ac = 8; }
					if ($std == 'E') { $std = 'Avaliação Finalizadas '; $ac = 9; }
		...
		if (round($this->plano_pibic_em) < 1)
			{
				...
				$sx .= '<input type="button" value="submeter novo plano PIBIC Júnior" onclick="'.$linkc.'" class="botao-geral" >';
			} else {
		...
		if ($planos >= 1)
			{
				...
				$sx .= '<input type="button" value="Finalizar submissão deste projeto e dos planos dos alunos >>> " onclick="'.$linkc.'" class="botao-confirmar" >';			
			}
	...
	function form_02b()
		{
			...
			$sx .= '<TR>'.sget("dd20",'$Q descricao:a_cnpq:select a_cnpq || \' \' || a_descricao as descricao,* from ajax_areadoconhecimento where a_semic = 1 order by a_cnpq','Área',1,1);
			return($sx);
		}
	function form_02c()
		{
			...
			$sx .= '<TR>'.sget("dd21",'$Q descricao:a_cnpq:select a_cnpq || \' \' || a_descricao as descricao,* from ajax_areadoconhecimento where a_cnpq like \'9%\' order by a_cnpq','Área',1,1);
			return($sx);			
		}
	function form_03a()
		{
	...
	function valida_post()
		{
			...
			$valid = 1;
			$err = array();
			if (strlen($dd[30]) ==0 ) { $valid = 0; array_push($err,'Status do CEP não informado.'); }
			if (strlen($dd[32]) ==0 ) { $valid = 0; array_push($err,'Status do CEUA não informado.'); }

			if (strlen($dd[25]) ==0 ) { $valid = 0; array_push($err,'Aprovação por orgão de fomento não informado.'); }
			if (strlen($dd[35]) ==0 ) { $valid = 0; array_push($err,'Aprovação por empresa não informado.'); }
			
			/* arquivo */
			if ($this->valida_arquivo('PROJ',$protocolo)==0)
			{ $valid = 0; array_push($err,'Projeto ou Plano não foi anexado.'); }	
	...
	function project_professor_dados()
		{
			...
			$sx .= $this->form_02c(); /* area estratégica */
			$sx .= '</table>';
			$sx .= '</fieldset>';
			...
			$sx .= $this->form_04a(); /* Comitê de ètica em pesquisa */
			$sx .= $this->form_04b(); /* Comitê de ética no uso de animais */
			$sx .= '</table>';
			$sx .= '</fieldset>';
...
//				}
	...
	function plano_troca_protocolo_mae($de,$para)
		{
			...
			if ($tit1 == $tit2)
				{ $sx .= '<TR><TD>Título do trabalho: <font color="green">OK</font>'; }
				else 
				{ $sx .= '<TR><TD>Título do trabalho: <font color="red">ERRO</font>'; $ok = 0; }
	
			if ($status1 == 'X')
				{ $sx .= '<TR><TD>Protocolo ativo: <font color="red">ERRO, protocolo está cancelado</font>'; $ok = 0; }
			...
			if ($ok==1)
				{
					echo 'Troca de protocolo Mãe ('.$this->plano_troca_protocolo($de,$para).')';
					echo 'Cancela protocolo vazio ('.$this->protocolo_cancela_sem_plano($de).')';
					echo 'Devolve para indicação ('.$this->protocolo_altera_status($de,'A').')';
				}
		}
	...
	function projetos_sumetidos_campus($campus,$ano,$edital)
				...
				if ($line = db_read($rlt))
					{
						...
						enviaremail('renefgj@gmail.com','','Indicação de avaliação PIBIC/PIBITI',$texto);
						
						if (strlen($email) > 0) { enviaremail($email,'','Indicação de avaliação PIBIC/PIBITI',$texto); }
						if (strlen($email2) > 0) { enviaremail($email2,'','Indicação de avaliação PIBIC/PIBITI (copia)',$texto); }
		...
		function status_tipos()
			{
				$st = array('@' => 'Em submissão (início)',
							'A' => 'Submissão',
							'B' => 'Enviado',
							'X' => 'Cancelado',
							'T' => 'Pendência para TI',
							'U' => 'Pendência para o Gestor',
							'!' => 'Devolver ao professor para correções');
				$cr = array('@' => 'orange',
							'A' => 'orange',
							'B' => 'Green',
							'X' => 'Red',
				...
				if ($line = db_read($rlt))
					{
						$ok = 1;
						$this->erros = '';
						if (strlen(trim($line['pj_titulo'])) < 5) { $ok = -1; $this->erros .= 'Título inválido ou muito curto<BR>'; }
						//if (strlen(trim($line['pj_area'])) < 8) { $ok = -2; $this->erros .= 'Área do conhecimento não definida<BR>'; }						
					} else {
						$ok = 0;
					}
				return($ok);
				...
				if ($line = db_read($rlt))
					{
						$ok = 1;
						$this->erros = '';
						if (strlen(trim($line['pj_titulo'])) < 5) { $ok = -1; $this->erros .= 'Título inválido<BR>'; }
						if (strlen(trim($line['pj_area'])) < 8) { $ok = -2; $this->erros .= 'Área do conhecimento não definida<BR>'; }
						if (strlen(trim($line['pj_ext_sn'])) ==0 ) { $ok = -2; $this->erros .= 'Existência de financiamento externo não informado<BR>'; }
						if (strlen(trim($line['pj_gr2_sn'])) ==0 ) { $ok = -2; $this->erros .= 'Existência de financiamento por empresas não informado<BR>'; }
						/** */
						if ($line['pr_ext_sn']=='S')
							{
							if (strlen(trim($line['pj_ext_local'])) ==0 ) { $ok = -2; $this->erros .= 'Financiador externo não informado<BR>'; }		
							if (strlen(trim($line['pj_ext_edital'])) ==0 ) { $ok = -2; $this->erros .= 'Edital do financiador externo não informado<BR>'; }		
							if (strlen(trim($line['pj_ext_valor'])) ==0 ) { $ok = -2; $this->erros .= 'Valor do financiador externo não informado<BR>'; }		
							}
						...
						$rline = db_read($rrlt);
						$total = $rline['total'];
						if ($total <= 0) { $ok = -3; $this->erros .= 'Arquivo com o projeto do professor não postado.'; }
					} else {
						$ok = 0;
					}
				return($ok);
		...
		function submit_plano()
			{
				...
				$icv = $dd[9];
				$ok = 1;
				if (strlen($dd[2]) < 10) { $ok=-1; $this->erros .= 'Título muito curto<BR>'; }
				...
				if ($yline = db_read($yrlt))
					{
						$ok=-2; $this->erros .= 'Título já lançado no sistema<BR>';
					}
				/*** Salva Dados */
				...
				if (strlen(trim($dd[10])) == 0) { $ok=-5; $this->erros .= 'Modalidade do edital não selecionada<BR>'; }
		...
		function mostra_plano_line($line)
			{
						...
						if ($icv == 1) { $icv = 'SIM'; } else { $icv = 'NÃO'; } 
						if ($edital == 'PIBICE') { $edital = "PIBIC_EM"; }
						
						$sx .= '<TR><TD class="lt0">título do plano de trabalho do aluno';
		...
		function projetos_lista($tp='')
			{
				...
				$sx .= '<TR>';
				$sx .= '<TH>Protocolo.<TH>Título do trabalho<TH>Área';
				$tot = 0;
				$tit = 'X';
				$pro = '';
				$profx = 'x';
		...
		function mostra($line)
			{
				global $user;
					...
					$sta .= $line['pj_status'];
					if ($sta == '@') { $sx .= '<font color="orange"><B>Em submissão</B><font>'; }
					if ($sta == 'A') { $sx .= '<font color="green"><B>Em submissão</B><font>'; }
					...
					$sx .= '<TR><TD colspan=2>';
					$sx .= '<font class="lt0">título do projeto do professor</font>';
					...
					$sx .= '<table class="tabela00" width="100%">';
						...
						$sx .= '<font class="lt0" align="right">área do conhecimento: </font>';
						$sx .= '<TD class="tabela01"><font class="lt3"><B>'.trim($line['pj_area']).' - '.trim($line['area1']).'</B></font>';
	
						$sx .= '<TR><TD align="right">';
						$sx .= '<font class="lt0" align="right">área estratégica: </font>';
						...
						$sx .= '<font class="lt0" align="right">Projeto aprovado externamente por uma agência de fomento: </font>';
						...
						*/
						...
						if ($cep == '')  { $cep = 'Não aplicável'; }
						if ($cep == 'A')  { $cep = 'Aprovado'; }
						if ($cep == 'S')  { $cep = 'Em submissão'; }
						if ($cep == 'E')  { $cep = 'Em apreciação'; }
						$cep = '['.$cep.']';

						$sx .= '<TR><TD align="right">';
						$sx .= '<font class="lt0" align="right">Comitê de Ética em Pesquisa: </font>';
						$sx .= '<TD class="tabela01"><font class="lt3"><B>'.$cep.'</B></font>';
						

						$ceua = trim($line['pj_ceua']);
						if ($ceua == '')  { $ceua = 'Não aplicável'; }
						if ($ceua == 'A')  { $ceua = 'Aprovado'; }
						if ($ceua == 'S')  { $ceua = 'Em submissão'; }
						if ($ceua == 'E')  { $ceua = 'Em apreciação'; }
						$ceua = '['.$ceua.']';

						$sx .= '<TR><TD align="right">';
						$sx .= '<font class="lt0" align="right">Comitê de Ética no Uso de Animais: </font>';
		...
		function resumo_planos_list_line($line)
			{
				...
				$tipo = trim($line['doc_edital']);
							...
							if ((($tp == '') or ($tp == '1') or ($tp == '3')) and ($tipo == 'PIBIC' or $tipo == 'PIBITI'))
								...
								$sx .= '<TR>';
								$sx .= '<TD>TÍTULO DO PLANO DO ALUNO:<BR><font class="lt3"><b>'.$line['doc_1_titulo'];
								$sx .= '<TR>';
		...
		function resumo_projeto_campus($ss='',$centro=0,$escola=1)
			{
					...
					$sx .= 'NO EDITAL DA INICIAÇÃO CIENTÍFICA - '.date("Y").' - CAMPI</h2>';
		...
		function resumo_planos_centro($ss='',$centro=0,$escola=1)
			{
					...
					if ($escola==1)
						...
						$sql .= "inner join pibic_submit_documento on pj_codigo = doc_protocolo_mae
								...
								where pj_ano = '".date("Y")."'
										and (doc_status <> 'X') 
										and (pj_status = 'B' or pj_status = 'C' or pj_status = 'D' or pj_status = 'T'  or pj_status = 'A' or pj_status = 'F')
								";
					...
					$sx .= '<TD class="tabela01" align="center"><B>'.($ttotale+$ttotalt+$ttotalp);
					$sx .= '</table>';
					$this->plano_pibic = $totalp;						
					$this->plano_pibiti = $totalt;
					$this->plano_pibic_em = $totale;
					...
					while ($line = db_read($rlt))
						{
							...
							if (($sts == 'T'))
							{
								$sx .= '<A HREF="#" onclick="newxy2(\'submit_edite.php?dd0='.$line['id_doc'].'&dd89='.trim($line['doc_protocolo_mae']).'&dd90='.trim($line['doc_protocolo']).'\',600,400);" class="link">editar título</A>';
							}
		...
		function submit_projeto()
			{
				...
				$obr = '&nbsp;<font color="red">(obrigatório)</font>';
				$sx .= '<style>
				...
				Submissão de projeto do professor 
				<fieldset><legend>Dados do projeto</legend>
					<table width=98%>
					<tr><TD class="lt0">'.$msg1.'TÍTULO DO PROJETO PRINCIPAL
					<TD><NOBR>Protocolo '.$proto.'
					<tr><td><input type="hidden" name="dd1" value="'.$dd[1].'">';
				$sx .= '<h2>'.$dd[1].'</h2>';

//					<tr><TD class="lt0">RESUMO (até 250 palavras)
//					<tr><td><textarea name="dd2" cols="80" rows="4">'.$dd[2].'</textarea>

//					<tr><TD class="lt0">Palavras-chave (até 5 palavras)
//**
  				 $sx .= '
 					<tr><TD class="lt2">Banco de projetos
					<TR><TD class="lt0">O Banco de Projetos, recém implantado na PUCPR, abriga em seu banco de dados todos os projetos desenvolvidos na PUCPR, em âmbito acadêmico.
						Desta forma você pode cadastrar automaticamente seu projeto somente confirmando a informação abaixo:
					<tr><td><select name="dd80">
							<option value="S" '.$dd80a.'>SIM, autorizado a inserção deste projeto no banco de projetos da PUCPR</option>
							<option value="N" '.$dd80b.'>NÃO, não quero inserir meu projeto no banco de projetos</option>
							</select>
					</table>
				</fieldset>
				
				<fieldset><legend>Área do conhecimento</legend>
					<table width=98%>
					<tr><TD class="lt0">'.$msg10.'Área do conhecimento (seja o mais específico possível) '.$obr.'
					<tr><td><select name="dd10">';
				$sx .= $ar->forma_area_especifica($dd[10]);
				$sx .= '</select>	

					<tr><TD class="lt0">Área estratégica?
				...
				/** Aprovação externa e Financiamento **/
				...
				$sx .= '
				<fieldset><legend>Aprovação & Financiamento Externo</legend>
					<table width=98%>
					
					<tr><TD class="lt2"><B>'.$msg20.'Projeto aprovado externamente por uma agência de fomento?</B> '.$obr.'
					<TR><TD class="lt1">
						<input type="radio" value="0" name="dd20" '.$dd20a.' onclick="dd20h();"> Não
					...
					<TR><TD>
						<div id="apro20" style="'.$apro20.'" class="lt0" >
							...
							<table width=98% class="lt1">
								...
								<TR><TD align="right">Período de vigência (ínicio)
									<TD><select name="dd28">'.$op28.'</select>
								<TR><TD align="right">Período de vigência (término)
				...
				for ($r=(date("Y")-5);$r < date("Y")+5;$r++)
					...
					<TR><TD class="lt1">
						<input type="radio" value="0" name="dd30" '.$dd30a.' onclick="dd30h();"> Não
				...
				/** Comitê de Ética **/
				$ops = array('::não aplicável','em submissão','em avaliação','aprovado');
				$opt = array('N','A','B','F');
				$opa = '';
				$opb = '';
				...
				$sx .= '
				<fieldset><legend>Comitês de Ética</legend>
					<table width="98%" class="lt1">
					<TR><TH>Comitê
						<TH><I>Status</I>
						<TH>Nº Parecer
					<TR><TD>Comitê de Ética em Pesquisa
						<TD><select name="dd50">
							'.$opa.'
							</select>
						<TD align="center"><input type="text" name="dd51" size=15 maxlength=15 value="'.$dd[51].'">
					<TR><TD>Comitê de Ética no Uso de Animais
						<TD><select name="dd52">
							'.$opb.'
							</select>
				...
				if ((strlen($dd[2]) > 0) and (strlen($dd[0])==0))
					{
						...
						if ($line = db_read($rlt))
							{
								$err = 'Um projeto com este título já existe, só é possível submeter projeto com títulos diferentes';
								$dd[4]='';				
							} else {
								$dd[4] = '1';
							}
		...
		function submit_projeto_new2()
			{
						...
						if (strlen($proto)==0)
							{
								$professor = $ss->user_cracha;
								$ano = date("Y");
								/* RN: Verifica se não existe este título */
								$sql = "select * from ".$this->tabela."
 										where pj_professor = '$professor'
 										and pj_ano = '$ano'
									";
								...
								if ($tit == 0)
								{
									...
									$sql = "insert into ".$this->tabela."
										...
										('sem título','$proto','$professor',$data,$data,'!')";
									$rlt = db_query($sql);
									...
									$_SESSION['protocolo'] = $proto;
									if (strlen($proto)==0)
										{ echo 'Erro na geração do protocolo'; exit; }
				...
				<table width=100%>
				<TR><TD colspan=2 class=lt5 align=center >
				<h3>Submissão de projeto do professor</h3> 
				<fieldset><legend>Dados do projeto</legend>
					<table width=98%>
					<tr><TD class="lt0">'.$msg1.'TÍTULO DO PROJETO PRINCIPAL '.$obr.'
