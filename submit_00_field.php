...
while ($line = db_read($rlt))
	{
	/* Dados do campos de edição */
	$name_id = $line['sub_codigo'];
	if ($xcod != $name_id)
	{
		$xcod = $name_id;	
	/* Recupera dados gravados se não tem ação */
	if (strlen($acao) == 0) 
		{
	...
	/* Armazena para gravação dados já inseridos */
	if (!(substr($CP1,0,2) == '$B'))
	...
	/* Gerar Opcao para PDF */
	if ($CP1=='$TEAM') 
		{ require("team.php"); $ed = true; }

	/* Cronograma */
	if ($CP1=='$CRONO') ///////////////// Página para autoreso
		{
			/* Cronograma */
			$s .= '<TR><TD colspan=2>';
	...
	if ($CP1=='$BUDGET') ///////////////// Página para autoreso
		{
			/* Budget */
			$s .= '<TR><TD colspan=2>';
	...
	if (substr($CP1,0,5)=='$FILE') ///////////////// Orçamento
		{
			...
			$s .= '<A HREF="javascript:newxy2(\'ged_upload.php?dd1='.$protocolo.'\',500,400);">';
			$s .= msg("upload_file");
			$s .= '</A>';
			$s .= '</div>';
			$s .= '</fieldset>';
	...
	if (strlen($acao) > 0) 
		{
		if (($obriga == 1) and (strlen($value)==0)) 
			{ $ok = -1; echo '<BR>'.msg('requered').'==>'.$CP2; }
		}
	$dx++;
	}
	}
