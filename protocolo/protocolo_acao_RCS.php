...
$cp[2] = array('$M', '', 'Decisão', False, True);
$cp[3] = array('$T80:5', '', '', True, True);

$op = '@:Manter aberto';
$op .= '&F:Indeferir solicitação';
$op .= '&C:Cancelar protocolo de serviço';
$op .= '&D:Deferir solicitação do recurso';
...
if ($form -> saved > 0) {
	...
	switch($ssta) {
		case 'F' :
			...
			//$pb -> cancela_plano_do_aluno($proto);
			echo '<BR>Análise do recurso de submissão: <font color="green">ok</font>';
			...
			$texto = troca($texto, '$DECISAO', mst($dd[3]));
		...
		case 'D' :
			...
			$texto = troca($texto, '$PROTOCOLO', $proto_nr);
			$texto = troca($texto, '$DECISAO', mst($dd[3]));
			$texto = '<img src="http://www2.pucpr.br/reol/img/email_ic_header.png"><BR>'.$texto.'<BR><img src="http://www2.pucpr.br/reol/img/email_ic_foot.png">';
