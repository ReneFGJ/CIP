	...
	if ($tipo == '4')
		...
		for ($x=0;$x <= count($menu); $x++)
				...
				if (!($seto == $xseto))
					...
					////////////////////////////////////////////
					$sc = '<TR><TD colspan="10" class="menu_item_ul">';
					$sc .= '<NOBR>'.$xseto.'&nbsp;</TD>';
					$sc .= '<TR><TD><UL>';
					$seto = $xseto;
					$xcol=0;
				...
				if (isset($menu[$x][2]))		
					{ 
					$link = '<A href="'.$menu[$x][2].'" class="menu_item_li">';
					if (strlen(trim($menu[$x][2])) == 0) { $link = ''; }
					$pre = ''; $pos = '';
					...
					if ($col == 0)
						{
							$cm1 .= $sc;
							$cm1 .= $pre.'<LI>'.$link.$menu[$x][1].'</A><BR>'.$pos; 
						} else {
							$cm2 .= $sc;
							$cm2 .= $pre.'<LI>'.$link.$menu[$x][1].'</A><BR>'.$pos; 
						}
					}
				}
