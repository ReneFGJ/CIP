...
while ($line = db_read($rlt))
	...
	if (strlen($dd[10]) > 0)
		{
			$sx .= '<TD>==>enviar';
			$ttt = troca($texto,'$parecerista',$nome);
			$ttt = troca($ttt,'$link',$link);
			//$admin_nome = 'Waldemiro Gremski';
			...
			enviaremail('renefgj@gmail.com',$emailadmin,'(copia) AVALIAÇÃO - PIBIC PUCPR',mst($ttt).'<BR><BR>email para: '.$us_email.' '.$us_email_alt);
			enviaremail('pibicpr@pucpr.br',$emailadmin,'(copia) AVALIAÇÃO - PIBIC PUCPR',mst($ttt).'<BR><BR>email para: '.$us_email.' '.$us_email_alt);
