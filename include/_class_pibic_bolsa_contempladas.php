...
class pibic_bolsa_contempladas
	{
		...
		function relatorio_parcial_nao_entregue($ano='',$tipo='IC')
			{
				global $ic,$dd,$http;
				if (isset($ic))
				...
				$sql .= "select *
							...
							where (pb_status <> 'C' and pb_ano = '".$ano."' and pb_relatorio_parcial < 20000101) 
							$wh
							order by pp_nome
							";
				$rlt = db_query($sql);
		...
		function alunos_ativos_ic($ano='',$tipo=1)
			{
				if($tipo==1)
					{ $wh = "and (pp_centro = 'DOUTORANDO' or pp_centro = '' or pp_centro = 'PUC PR CAMPUS CURITIBA' or pp_centro = 'PUCPR CAMPUS SJP')"; }
				if($tipo==2)
					{ $wh = "and not (pp_centro = 'DOUTORANDO' or pp_centro = '' or pp_centro = 'PUC PR CAMPUS CURITIBA' or pp_centro = 'PUCPR CAMPUS SJP')"; }
	...
	function actions($ac)
		{
			...
			if ($ac == '100')
				{
					$cp = array();
					array_push($cp,array('$S8','pb_protocolo','Protocolo',False,False));
					array_push($cp,array('$HV','',$dd[1],False,False));
					array_push($cp,array('$HV','',$dd[2],False,False));
...
//					exit;
	...
	function ativos_por_bolsa($bolsa,$credito,$ano=2012,$tipo='')
		{
			global $LANG;
				...
				$sql = "
					...
					where (pb_status <> 'C' and pb_status <> 'F')
					and $wh pb_ano = '$ano'
	...
	function semic_area_de_apresentacao($ano=0)
		{
	...
	function semic_area_de_apresentacao_geral($ano=0)
		{
			if ($ano == 0) { $ano = (date("Y")-1); }
			$sql = "
				select * from (
	...
	function acompanhamento_idioma()
		{
	...
	function acompanhamento_relatorio_final()
		{
	...
	function orienacao_pibic_programa($programa_pos,$areas,$programa_pos_anoi=1900,$programa_pos_anof=2999)
		{	
			$sql = "select * from (			
						...
						inner join pibic_bolsa_tipo on pbt_codigo = pb_tipo
						inner join programa_pos_docentes on pdce_docente = pb_professor
			...
			$sx .= '<TR><TH>Prof.<TH>Ano<TH>PIBIC<TH>PIBITI<TH>PIBIC_EM<TH>Inclusóo<TH>NA<TH>CSF<TH>Outros';
			while($line = db_read($rlt))
				{
					...
					if ($xano != $ano)
						{ $sx .= '<TR><TD0><h3><center>'.$ano.'</H3>'; $xano = $ano;}					
	...
	function orientacao_pibic($programa_pos,$areas,$programa_pos_anoi=1900,$programa_pos_anof=2999)
		{
			$sql = "select * from pibic_bolsa_contempladas
					    inner join pibic_bolsa_tipo on pbt_codigo = pb_tipo
						inner join ( select pdce_docente from programa_pos_docentes where pdce_programa = '".$programa_pos."') as docentes2 on pdce_docente = pb_professor ";					    					  
			$sql .= " where ";
			...
			$sa .= '<TH width="10%">PIBIC<TH width="10%">PIBITI<TH width="10%">PIBIC_EM(Jr)
					<TH width="10%">Inclusóo<TH width="10%">NA<TH width="10%">Outros';
			$sa .= $sb;
		...
		function mostra_relatorios_status()
			{
				...
				$ns = array('-','aprovado','nóo aprovado','nao avaliado','','','','','','-','');
				...
				<TR class="lt2" bgcolor="#F0F0F0" align="center">
					...
					<TD width="17%">Correóoo R. Parcial </TD>
		...
		function mostra_registro($line)
			{
				...
				{
					if ($line['pb_relatorio_nota'] == 0) { $sr .= ' parcial <font color="#ff00ff">nóo avaliado'; }
					if ($line['pb_relatorio_nota'] == 1) { $sr .= 'parcial <font color=green><B>aprovado</B>'; }
					if ($line['pb_relatorio_nota'] == 2) { $sr .= 'parcial <font color=red><B>pendóncias</B>'; }
					if ($line['pb_relatorio_nota'] == -1) { $sr .= 'parcial <font color="orange"><B>enviado para correções</B>'; }
				} else {
		 			$sr .= 'parcial <font color="orange"><B>nóo enviado</B>'; 
				}
				$sr .= '</TR>';				
				return($sr);
			}
			...
			if ($d1 <= 19000101) 
				{
					...
					if ($n1 == 1) { $sx .= ' '.msg('aprovado'); }
					if ($n1 == 2) { $sx .= ' '.msg('accept'); }
					if ($n1 == 3) { $sx .= ' '.msg('reproved'); }
				}
			return($sx);
		}
