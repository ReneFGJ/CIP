...
function brtos($dt)
{  
	$dt = round(substr($dt,6,4).substr($dt,3,2).substr($dt,0,2));
	if ($dt == 0) { $dt = 19000101; }
	return($dt);
}

function stodbr($dt)
{ global $date,$LANG; $LANG ="pt_BR";
...
function nomemes($dx)
	{ global $date; return(month_name($dx,1)); }
	
function nomemes_short($dx)
	{ global $date; return(month_name($dx,2)); }
