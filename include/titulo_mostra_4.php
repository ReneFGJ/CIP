...
if ($mostra_issue != True)
	...
	if (strlen($issue) > 0)
		...
		if (strlen($seccao) > 0) { $sql = $sql . " and article_section = ".sonumero($seccao)." "; }
...
while ($line = db_read($result))
	...
	if ($ida != $s2)
		...
		echo "<TR><TD><BR></TD></TR>";
		echo '<TR valign="top">';
		echo '<TD width=10 align="right" class="texto">&nbsp;'.$nr.'.&nbsp;';
		echo '</TD>';
		if ($art > 1)
			{
			echo '<TD colspan=5 class="texto2">';
			}
			else
			{
			echo '<TD colspan=3 class="texto2">';
			}
		$xlink = '<A HREF="?dd1='.$line['id_article'].'&dd99=view">';
		$tit_alt = trim($line['article_2_title']);
		echo $xlink.'<font class="texto3">'.$line['article_title'].'</B></a></font>';
		if (strlen($tit_alt) > 0)
			{
			echo '<BR><img src="'.$img_dir.'nada.gif" width="12" height="12" alt="" border="0">';
			echo $xlink.'<BR><I><font class="texto2">'.$tit_alt.'</B></a></I>';
			echo '<BR><img src="'.$img_dir.'nada.gif" width="12" height="12" alt="" border="0">';
			}
		echo '</TD>';
		echo '</TR>';
		...
		if (strlen(trim($autor)) > 1)
			...
			if ($art > 1)
				{
				echo '<TD width=10 >&nbsp;</TD>';
				echo '<TD class="texto" colspan="4"><I>';
				}
				else
				{
				echo '<TD class="texto" colspan="3"><I>';
				}
			echo $autor,'</TD>';
			}
		///////////// Numero do Periódico
		...
		if (strlen(trim($page)) > 0)
			{
			echo '<TD class="texto2" align="right" colspan="2"><I>';
		...
		if (($art == 1) and (strlen($capa) > 0))
			...
			<TD bgcolor="<?=$ttt?>"><img src="<?=$img_dir?>nada.gif" width="30" height="13" alt="" border="0"></TD>
			<TD bgcolor="<?=$ttt?>"><img src="<?=$img_dir?>nada.gif" width="80" height="13" alt="" border="0"></TD>
			</TR>			
			<?
			}
		}
