...
if ($mostra_issue != True)
	...
	if (strlen($issue) > 0)
		...
		$mais = 20;
		if ($dd[5] >= 7) { $mais = 0; }
		$sql = "select * from sections where sections.seq = ".sonumero($dd[5]);
		$sql .= " and journal_id = ".$jid;
		$rlt = db_query($sql);
		$line = db_read($rlt);
		echo '<center><font class="lt5">'.$line['title'].'</font></center><BR>';
...
//		if (strlen($seccao) > 0) { $sql = $sql . " and article_section = ".$seccao." "; }		
		...
		if (strlen($dd[5]) > 0) { $sql .= " and sections.seq = ".($dd[5]+$mais); }
		
		$sql .= " and article_publicado <> 'X' ";
		$sql = $sql . ' order by seq,title, issue_title,sections.seq, article_seq';
		$result = db_query($sql);
		$ttt="#FFFFFF";
		//$ttt="#20c0ff";
