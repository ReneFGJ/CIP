...
//	$sx .= $img_estrategia;
	...
	$sx .= '<TD>';
	$sx .= dsp_sn($line['doc_tipo']);
	
	$sx .= '<TD><NOBR>';
	
	if ($line['pee_icv']=='1')
		{
			$sx .= '<A onclick="newxy(\''.$ptipo.'_bolsa_indicar.php?dd5=I&dd1='.trim($line['pp_cracha']).'&dd2='.trim($line['doc_protocolo']).'&dd0='.trim($line['doc_aluno']).'\',400,300);">';
			$sx .= '[ICT]';
			$sx .= '</A>';
			...
			$sx .= '<A onclick="newxy(\''.$ptipo.'_bolsa_indicar.php?dd5=B&dd1='.trim($line['pp_cracha']).'&dd2='.trim($line['doc_protocolo']).'&dd0='.trim($line['doc_aluno']).'\',400,300);" class="lta">';
			$sx .= '[CNPq]';
			$sx .= '</A>';
			$sx .= '<A onclick="newxy(\''.$ptipo.'_bolsa_indicar.php?dd5=O&dd1='.trim($line['pp_cracha']).'&dd2='.trim($line['doc_protocolo']).'&dd0='.trim($line['doc_aluno']).'\',400,300);" class="lta">';
			$sx .= '[PUCPR]';
			$sx .= '</A><BR>';
			$sx .= '<A onclick="newxy(\''.$ptipo.'_bolsa_indicar.php?dd5=G&dd1='.trim($line['pp_cracha']).'&dd2='.trim($line['doc_protocolo']).'&dd0='.trim($line['doc_aluno']).'\',400,300);" class="lta">';
			$sx .= '[AGENCIA-PUCPR]';
			$sx .= '</A>';
			$sx .= '<A onclick="newxy(\''.$ptipo.'_bolsa_indicar.php?dd5=V&dd1='.trim($line['pp_cracha']).'&dd2='.trim($line['doc_protocolo']).'&dd0='.trim($line['doc_aluno']).'\',400,300);" class="lta">';
			$sx .= '[APROV.]';
			$sx .= '</A>';
		}
	}
