...
if ($saved > 0)
		...
		$sql = "select * from $tabela where doc_protocolo = '".$dd[1]."' ";
		$rlt = db_query($sql);
		$line = db_read($rlt);
		...
		$sql = "select * from $tabela where doc_protocolo = '".$dd[2]."' ";
		$rlt = db_query($sql);
		$line = db_read($rlt);
		...
		/// Ponto 1
		echo '<tr '.coluna().'><TD width="20">1</TD><td>Títulos do projeto do professor igual</td>';
		if ($tit_1 == $tit_2)
		...
		/// Ponto 6
		echo '<tr '.coluna().'><TD width="20">1</TD><td>Status em do projeto em (A ou B)? ('.$sta_1.')</td>';
		if (($sta_1 == 'A') or ($sta_1 == 'B'))
			{ echo '<TD><font color=green>OK</TD>'; } else { echo '<TD><font color=red>Não OK!</font></TD>'; $err++; }
			
		/// Ponto 7
		echo '<tr '.coluna().'><TD width="20">1</TD><td>Protocolo #1 é projeto de professor</td>';
		if (strlen($mae_1) == 0)
			{ echo '<TD><font color=green>OK</TD>'; } else { echo '<TD><font color=red>Não OK!</font></TD>'; $err++; }

		/// Ponto 8
		echo '<tr '.coluna().'><TD width="20">1</TD><td>Protocolo #2 é projeto de professor</td>';
		if (strlen($mae_2) == 0)
			{ echo '<TD><font color=green>OK</TD>'; } else { echo '<TD><font color=red>Não OK!</font></TD>'; $err++; }
