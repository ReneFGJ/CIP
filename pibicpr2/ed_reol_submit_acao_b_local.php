...
$qsql .= ") ";
$qsql .= " and us_ativo = 1 ";
$qsql .= " and us_instituicao = '0000232' ";
$qsql .= " and us_journal_id = 20 ";
$qsql .= "order by us_nome,a_cnpq ";


$qrlt = db_query($qsql);
$sp = '<TR><TD class="lt3" colspan="5" align="center"><HR>COMITÊ LOCAL<HR></TD></TR>';
...
//	echo '<BR>'.UpperCaseSql(trim($qline['us_nome']));
	...
	$sp .= '<TR>';
	$sp .= '<TD>';
	$sp .= '<input type="radio" name="dd21" value="'.$qline['us_codigo'].'" '.$chk.'>';
	$sp .= '&nbsp;';
	$sp .= $qline['total'];	
	$sp .= '<TD>';
	$sp .= $se;
