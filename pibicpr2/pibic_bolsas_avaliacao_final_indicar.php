...
$sql .= " inner join pareceristas_area on pa_parecerista = us_codigo ";
$sql .= " inner join ajax_areadoconhecimento on a_codigo = pa_area ";
$sql .= " inner join instituicoes on us_instituicao = inst_codigo ";
$sql .= " left join (select count(*) as pareceres, pp_avaliador as avaliador from pibic_parecer_2010 where pp_tipo = '".$tipp."' group by pp_avaliador ) as ttt on pa_parecerista = avaliador ";
$sql .= " where ";
//$sql .= " where (pp_protocolo = '".$protocolo."') or (pp_protocolo_mae = '".$protocolo."')";
//$sql .= " and (pp_status = 'B') ";
$sql .= " us_instituicao = '0000232' ";

$sql = "select * from pareceristas ";
$sql .= " left join (select count(*) as pareceres, pp_avaliador as avaliador 
		from pibic_parecer_".date("Y")." where pp_tipo = '".$tipp."' group by pp_avaliador ) as ttt on us_codigo = avaliador ";
$sql .= " inner join instituicoes on us_instituicao = inst_codigo ";
$sql .= " inner join pareceristas_area on pa_parecerista = us_codigo ";
$sql .= " inner join ajax_areadoconhecimento on a_codigo = pa_area ";
...
if ((strlen($dd[10]) > 0) and (strlen($dd[11]) > 0))
	...
	if ($line = db_read($rlt))
		...
		$key = substr(md5($protocolo.$idp),5,10);
		...
		$texto = troca($texto,'$professor',$prof_nome);
