...
while ($line = db_read($rlt))
	...
	if ($professor != $xprof)
		...
		if (($pibic + $pibiti) > 0)
			...
			if ($pibiti > 0)
				{
					$sx .= '<TD colspan="'.(7-($pibiti+$repo)).'">&nbsp;</TD>';
				} else {
					$sx .= '<TD colspan="'.(15-($pibic-$pibiti))	.'">&nbsp;</TD>';
				}
			$sx .= '<TD align="center"  '.$bgp.'>'.$pibic.'/'.$pibic_r.'</TD>';
			$sx .= '<TD align="center" '.$bgt.'>'.$pibiti.'/'.$pibiti_r.'</TD>';
			$sx .= '<TD>'.$msg.'</TD>';
			}
		}	
	...
	if ($centro != $xcentro)
		{
		$sx .= '<TR class="lt3"><TD colspan="40"><B><font style="font-size:22px">'.$centro.'</font></B></TD></TR><TR>';
		$xcentro = $centro;
		}
	...
	if ($professor != $xprof)
		{
		$sx .= '<TR class="lt2"><TD colspan="1">'.$professor.'</TD>';
		$sx .= '<TD>'.$tit.'</TD>';
