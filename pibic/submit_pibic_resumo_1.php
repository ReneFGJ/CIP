...
//	echo '<HR>';
	...
	$xsql .= " and spc_codigo = '00092' ";
	$xrlt = db_query($xsql);
	if ($xline = db_read($xrlt)) { $pl1_icv = trim($xline['spc_content']); }

	$spp .= $nl . '<TABLE width="'.$tab_max.'" align="center" class="lt2" border=0 >';
	$spp .= $nl . '<TR bgcolor="#e0e0e0"><TH colspan="3">Plano de trabalho PIBIC (Aluno)</TH></TR>';
	...
	if ($ed_acao != false)
		...
		$spp .= $nl . '	<input type="hidden" name="dd0" value="00015">';
