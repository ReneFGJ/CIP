...
require("submit_table_pibic.php");
...
if ((strlen($protocolo) == 0 ) and ($prj_pg != 1))
	{
	redirecina('submit_phase_2_pibic.php?dd98=1');
	}
...
$s .= '<TR><TD colspan="2"><center><img src="img/logo_pibic.jpg" width="423" height="52" alt="" border="0"></center></TD></TR>';
$dx=0;
$ok = 1;
$cops = array();
...
//	$estilo = trim($sb_line['sub_estilo']);
	...
	if (substr($CP1,0,6)=='$PIBITI') ///////////////// Orçamento
		{ require("submit_pibiti_resumo.php"); $ed = true; echo $sr; }
				
	if (substr($CP1,0,5)=='$FIM') ///////////////// Finaliza
		{ redirecina("submit_finalizar_2.php?dd0=SIM"); $ed = False; exit;}
...
if (($erro) or ($nerr > 0))
	{
		if ($nerr >0)
			{ require("submit_pendencias.php"); require("foot.php"); exit; }
	} else {	
...
//	require("submit_phase_2_fields.php");
	...
	redirecina("submit_phase_2_pibic.php?dd98=".($prj_pg+1));
	exit;
	} else {
		///////////////// Liberar Gravação
		echo '';
