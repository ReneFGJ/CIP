...
class submit
	...
	function documents_requeried_old()
		{
			global $jid;
			$sx = 'xxx';
			...
			while ($line = db_read($rlt))
				{
					...
					$sx .= '<TR><TD>';
					$sx .= '<input type="button" value="'.msg('post_file').'">';
				}
			$sx .= '</table>';
			return($sx);
		}
	...
	function mostra_autor($page='')
		{
			$email = trim($this->author_email);
			$email2 = trim($this->author_email_alt);
			$nome = $this->author_nome;
			$codigo = $this->author_codigo;
			...
			$sx .= '<fieldset><legend>'.msg('author_information').'</legend>';
			$sx .= '<table class="lt1" 
						width="100%" border=0 
						cellpadding=0 cellspacing=1 
						>';
			$sx .= '<TR><TD class="lt0" align="right" width="5%"><NOBR>'.msg('author');
			$sx .= '	<td class="lt2"><b>'.$nome.'</B> ';
			$sx .= '	<td class="lt0" align="right"><b>'.$codigo.'</B> ';
			if (strlen($email) > 0)
				{
					$sx .= '<TR>
							<TD class="lt0" align="right" width="5%"><NOBR>e-mail
							<TD class="lt2"><B>'.trim($email).'</B>';
				}
			if (strlen($email2) > 0)
				{
					$sx .= '<TR>
							<TD class="lt0" align="right" width="5%"><NOBR>e-mail alternativo
							<TD class="lt2"><B>'.trim($email2).'</B>';
				}				
			$sx .= '<TR><TD>::';
			$sx .= trim($this->author_line['sa_instituicao_text']);
			
			$sx .= '<TR><TD align="left" colspan=5>';
			$sx .= '<A HREF="'.$page.'" class="botao-geral" >';
			$sx .= msg('edit');
			$sx .= '</A>';
	...
	function cp_user()
		{
			...
			array_push($cp,array('$HV','','01',True,True));
			array_push($cp,array('$A8','',msg('persona_information'),False,True));
			array_push($cp,array('$S100:submit_autor','sa_email',msg('email'),True,False));
			array_push($cp,array('$ALERT','',$this->erro,False,True));
			array_push($cp,array('$S100','sa_nome',msg('name'),True,True));
			array_push($cp,array('$Q ap_tit_titulo:ap_tit_codigo:select * from apoio_titulacao where at_tit_ativo = 1 order by ap_tit_titulo','sa_titulo',msg('titulation'),True,True));
			array_push($cp,array('$S100','sa_lattes',msg('lattes'),False,True));
				...
				if ($line = db_read($rlt))
					{
						...
						enviaremail('renefgj@gmail.com','',msg('recover_password'),$texto);				
						echo '
		...
		function autor_login_form($page='')
			{
				...
				$sx .= '<TR><TH colspan=2>'.msg('sub_login_title_'.strzero($jid,4));
