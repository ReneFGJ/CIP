...
//XXX Idealmente, daria para testar com file_get_contents(filename) para ver se o arquivo de localização do datepicker 
//	existe antes de puxar ele aqui (e carregando jquery.ui.datepicker-pt-BR.js caso não exista)
...
/*
 * Dependências do dynatree e do tinymce
 */
echo '
...
class form
	{
		...
		/**
		 * Índice de $cp onde se encontram parâmetros (opcionais) de um tipo
		 *  Usado, e.g., em type_ARV() para receber a árvore
		 * @var integer
		 */
		var $indiceParams = 5;
		...
		function process($cp)
			{
				...
				switch ($i) 
				{
					...
					case 'ARV': /* Árvore com checkboxes */
		 ...
		 * @param  array   $arvore       	   uma árvore no formato ($chv, $nome, $filhos)
		 * @param  array   $chavesSelecionadas Chaves que serão selecionadas na inicialização
		 * @param  boolean $expandirRaiz 	   Expande a visualização da raiz por padrão
		 * @return string                	   a árvore expandida no formato esperado pelo dynatree
		 */
		function _type_ARV_expande_arvore($arvore, $tokenSepFormArvore, $expandirRaiz=true)
			{
				...
				if(strpos($chv, $tokenSepFormArvore) !== false){
					die("ERRO: Chave inválida por contém separador de chaves: $chv");
				}
		...
		/**
		 * Árvore com checkboxes para seleção
		 * Aqui usando o dynatree: http://code.google.com/p/dynatree/
		 * @param  array $arvore uma árvore no formato ($chv, $nome, $filhos)
		 * @return string        html/js de uma árvore com checkboxes selecionáveis
		 */
		function type_ARV($arvore, $tokenSepFormArvore='%%')
			{
				...
				// Persistência de seleção (entre POSTs, etc.)
				$sel .= "
						preSelecionar = {}; ";
				if($this->value){
		...
		function type_RT()
			{
				...
				die('Tipo de gerador de campo não suportado: '.$this->geradorCampoRichText);
			}

		/**
		 * Campo de seleção de tags com autocomplete
		 * http://jqueryui.com/autocomplete/#multiple-remote
		 * @param  string $fonteDados    -Se for uma string começada com 'http', é tratada como uma URL que retorna uma lista de dados
		 *                                 no formato JSON (objetos com atributos 'label' e 'value')
		 *                               -Se for uma string começada com 'function', é tratada como função de autocomplete segundo documentação em http://api.jqueryui.com/autocomplete/
		 *                               -Se for um array, é tratada como uma lista de tags 
		 * @param  string $jsTermFun     Se especificado, a função javascript f(request.term) é chamada na inserção de um termo no campo (ver jsAutocompleteSource)
		...
		function type_ATAGS($fonteDados, $jsTermFun='')
			{
				...
				if(is_array($fonteDados)){
					...
					foreach($fonteDados as $tag){
						...
						else{ die("ERRO type_ATAGS(): Tipo de tag inválido."); }

						if(!preg_match('/^[#_a-z][_a-z0-9]*$/', $label)){
							var_dump($tag);
							var_dump(debug_backtrace(false));
							die('ERRO type_ATAGS(): Apenas tags alfanuméricas minúsculas (sem acentos) começadas com uma letra ou cerquilha (#) são suportadas.');
						}
						if(preg_match('/\'/',$value)){
							die("ERRO type_ATAGS(): valor de tag possui caracter inválido: \'.");
						}
				...
				elseif(is_string($fonteDados) && preg_match("/^\s*function/", $fonteDados)){
					...
					var_dump($fonteDados);	
					die('ERRO type_ATAGS(): Fonte de dados inválida, vazia ou não suportada.');
				}
