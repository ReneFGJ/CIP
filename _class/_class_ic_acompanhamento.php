...
class ic_relatorio
	...
	function idicacao_avaliador_email($tst)
		{
			global $dd;
			require_once("../_class/_class_ic.php");
			$ic = new ic;
			...
			if (1==0)
			{
				$sql = "delete from ".$this->tabela." where pp_tipo = 'RPAR' ";
				$rlt = db_query($sql);
			...
			$sql = "update ".$this->tabela." set pp_data = 20130310 where pp_tipo = 'RPAR' ";
			$rlt = db_query($sql);
	...
	function idicacao_avaliador()
		{
			...
			$sql = "select * from pibic_bolsa_contempladas
					left join ".$this->tabela." on pb_protocolo = pp_protocolo and (pp_status <> 'X' and pp_status <> 'D')
					...
					where (pb_relatorio_parcial_nota <> 1 and pb_relatorio_parcial_nota <> 2)
					and (pb_relatorio_parcial > 20100101) and (pb_status <> 'C' and pb_status <> 'F')
					and (pb_ano = '".date("Y")."' or pb_ano = '".(date("Y")-1)."')
					$wh and (pp_status isnull)
					order by a_cnpq, pp_nome, pb_protocolo
			";
			$rlt = db_query($sql);
	...
	function lista_relatorios_pendentes_correcoes($id_pesq)
		{
				...
				while ($line = db_read($rlt))
				{
					...
					$aluno = $line['pa_nome'];
					$status = $line['pb_status'];
					$link = '<a href="atividade_IC2_acao.php?dd0='.trim($line['pb_protocolo']).'&dd1='.$line['id_pb'].'&dd90='.checkpost(trim($line['pb_protocolo'])).'">';
					$sx .= ''.$link.$ttp.'</A>';
					$sx .= '<BR>';
					$sx .= $aluno;
					$sx .= '<BR>Modalidade: '.$bolsa.' ('.$bolsa_nome.'/'.$line['pb_ano'].')';
					$sx .= '<TD>';
					$sx .= '<form action="atividade_IC2_acao.php">';
					$sx .= '<input type="hidden" name="dd0" value="'.trim($line['pb_protocolo']).'">';
					$sx .= '<input type="hidden" name="dd1" value="'.trim($line['id_pb']).'">';
					$sx .= '<input type="hidden" name="dd90" value="'.checkpost(trim($line['pb_protocolo'])).'">';
					$sx .= '<input type="submit" value="Entregar" class="botao-geral">';
