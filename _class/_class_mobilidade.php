...
class mobilidade {
	...
	function lista_visitante($tipo = '', $a1 = 1990, $a2 = 2999, $ed = 1) {
		//$this->strucuture();
		if (strlen($tipo) > 0) { $wh .= "where mt_publico = '$tipo' ";
		}
		...
		while ($line = db_read($rlt)) {
			...
			if (($di >= $a1) and ($di <= $a2)) {
				...
				if ($ed == 1) {
					...
					if ($tipo == 'V') {
						$link = '<A HREF="mobilidade_visitante_ed.php?dd0=' . $line['id_mb'] . '&dd90=' . checkpost($line['id_mb']) . '" class="link">';
					}
					if ($tipo == 'E') {
						$link = '<A HREF="mobilidade_estrangeiro_ed.php?dd0=' . $line['id_mb'] . '&dd90=' . checkpost($line['id_mb']) . '" class="link">';
					}
					$sx .= $link . 'editar</A>';
					$ln = $line;
				}
	...
	function lista_mobilidade($tipo = '', $a1 = 1990, $a2 = 2999, $ed = 1) {
		//$this->strucuture();
		$sx .= '<H3>' . $a1 . ' - ' . $a2 . '</h3>';
		if (strlen($tipo) > 0) { $wh .= "where mt_publico = '$tipo' and mb_status <> 'X' ";
		} else { $wh .= "where mb_status <> 'X' ";
		}
