...
class bi {
	...
	function professor_com_captacao($tp = 0) {
		$ano = $this -> ano_ini;
		$wh = '';
		$tit_rel = '% Professores com Projetos de Captação Aprovados / Andamento';
		$cp = "";
		...
		$rlt = db_query($sql);
		$id = 0;
		if ($line = db_read($rlt)) { $ProfCap = $line['total'];
		}
	...
	function producao_docente_scimago($anoi = 0, $anof = 0) {
		if ($anoi == 0) { $anoi = date("Y");
		}
		if ($anof == 0) { $anof = date("Y");
		}
		...
		while ($line = db_read($rlt)) {
			...
			$programa_pos_anof = $anof;
			$sx .= '<TR><TD colspan=4><TD class="tabela00" colspan=5>' . $line['pos_nome'];
			$sx .= '<TR><TD colspan=4><TD>' . $lattes -> resumo_scimago_ss($programa_pos, $areas, $programa_pos_anoi, $programa_pos_anof, 1);
		}
		$sx .= '<TR><TD colspan=4><TD colspan=5>' . $lattes -> formula_equivalencia();
		$sx .= '</table>';
		return ($sx);
