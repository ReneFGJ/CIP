		...
		function insert()
			{
				...
				'Parecer do Relatório Parcial','PRP',1,
				1,1,1,
				1
				)";
				$rlt = db_query($sql);
		...
		function cp_type()
			{
				...
				array_push($cp,array('$S3','doct_codigo','Código',False,True));
		...
		function upload_botton_with_type($proto='',$base='',$tp='',$bt='')
			{
				...
				if (strlen($tp) == 0)
					{ $sx .= '<option>::'.msg('document_type').'::</option>'; }

				$sql= "select * from ".$this->tabela.'_tipo where ';
				if (strlen($tp) > 0) 
					{ $sql .= " (doct_codigo = '".$tp."' ) and "; }
				...
				while ($line = db_read($rlt))
					{
						...
						$sx .= '<TD class="tabela01">'.$link.$line['doc_filename'].'</A>';
