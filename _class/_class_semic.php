...
class semic {
	...
	function pibic_semic_participacao($tipo = '', $modalidade = '') {
		...
		$wh = '';
		$modalidade = 'O';
		if (strlen($modalidade) > 0) { $wh .= " where av_tipo_trabalho='" . $modalidade . "' ";
		}
	...
	function autor_tipo($funcao) {
		switch ($funcao) {
			...
			case "4" :
				...
				break;
			case "5" :
				$funcao = "Supervisor Pibic Junior";
				break;
			case "6" :
				$funcao = "Escola";
				break;
	...
	function mostra_projetos_area($ano = '') {
		global $dd;
		if (strlen($dd[1]) > 0) {
			if ($dd[1] == 'PIBIC') { $wh = " and ( pbt_edital = 'PIBIC' or pbt_edital = 'IS') ";
			}
			if ($dd[1] == 'PIBITI') { $wh = " and ( pbt_edital = 'PIBITI' ) ";
			}
			if ($dd[1] == 'PIBICE') { $wh = " and ( pbt_edital = 'PIBICE' ) ";
			}
		}
	...
	function postar_resumo_ingles() {
		...
		$sx .= '<input type="hidden" name="dd0" value="' . $dd[0] . '">
					...
					<input type="submit" class="botao-geral" value="' . ('enviar revis„o') . '" id="bt_post">';
		$sx .= '</form>';
		$sx .= '</div>';

		$sx .= '<input type="button" class="botao-geral" value="' . ('postar revis„o') . '" id="bt_revisao">';
		$sx .= '
				<script>
		...
		if (strlen($dd[0]) > 0) {
			...
			if ((strlen($acao) > 0) and (strlen($dd[1]) > 0)) {
				if ($dd[1] == 'C') {
					...
					echo '<HR>Enviado para correÁıes<HR>';
				}
			...
			if ($this -> line['sm_status'] == 'A') {
				$sx .= '<TR><TD><input type="radio" name="dd1" value="X"> Cancelar submiss„o';
				$sx .= '<TR><TD><input type="radio" name="dd1" value="B"> aceitar submiss„o';
				$sx .= '<TR><TD><input type="radio" name="dd1" value="C"> enviar para correÁıes';
			}
			...
			if ($this -> line['sm_status'] == 'D') {
				$sx .= '<TR><TD><input type="radio" name="dd1" value="B"> Indicar para nova revis„o';
				$sx .= '<TR><TD><input type="radio" name="dd1" value="B"> aceitar submiss„o';
			}
			...
			if ($this -> line['sm_status'] == 'X') {
				$sx .= '<TR><TD><input type="radio" name="dd1" value="A"> Reenviar para an·lise';
				$sx .= '<TR><TD><input type="radio" name="dd1" value="B"> aceitar submiss„o';
			}
			if ($this -> line['sm_status'] == '@') {
				$sx .= '<TR><TD><input type="radio" name="dd1" value="A"> Enviar para an·lise';
				$sx .= '<TR><TD><input type="radio" name="dd1" value="B"> aceitar submiss„o';
				$sx .= '<TR><TD><input type="radio" name="dd1" value="X"> Cancelar submiss„o';
			}
	...
	function semic_resumo_checar($id) {
		...
		$rlt = db_query($sql);
		if ($line = db_read($rlt)) {
			if (strlen($line['sm_rem_01']) == 0) { $erro .= 'Falta <B>IntroduÁ„o</B>.<BR>';
			}
			...
			if (strlen($line['sm_rem_11']) == 0) { $erro .= 'Falta <B>IntroduÁ„o</B> em InglÍs.<BR>';
			}
			if (strlen($line['sm_rem_12']) == 0) { $erro .= 'Falta(m) o(s) <B>Objetivo(s)</B> em InglÍs.<BR>';
			}
	...
	function semic_mostrar($id, $ed2 = '') {
		...
		if ($line = db_read($rlt)) {
			...
			$this -> id = $line['id_sm'];
			$cod = $line['sm_codigo'];
			if (strlen($cod) > 0) { $wha = " sma_protocolo = '" . $cod . "' ";
			} else { $wha = " sma_protocolo = '" . strzero($id, 7) . "' ";
			}
			...
			$resumo = $link01 . '<B>IntroduÁ„o:</B> ' . $linkx . $line['sm_rem_01'];
			$resumo .= $link02 . ' <B>Objetivos:</B> ' . $linkx . $line['sm_rem_02'];
			$resumo .= $link03 . ' <B>MÈtodo:</B> ' . $linkx . $line['sm_rem_03'];
			...
			if ((strlen(trim($line['sm_rem_05'])) > 0) or ($edit == 1)) {
				$resumo .= $link05 . ' <B>Conclus„o:</B> ' . $linkx . $line['sm_rem_05'];
			}
	...
	function submit_list($status, $tipo = '', $page = '') {
		...
		while ($line = db_read($rlt)) {
			...
			if ($idm != $xidm) {
				...
				if ($view == 1) { $linkv = '<A HREF="semic_submissao_detalhe.php?dd0=' . $line['id_sm'] . '&pag=1&dd90=' . checkpost($line['id_sm']) . '" class="link">';
				}
	...
	function submit_resume_mostra($page = '') {
		...
		$sx .= '<TR><TD colspan=5>';
		$sx .= '<h1>Resumo das Submissıes</h2>';
		$sx .= '<TR>';
		$sx .= '<TH width="17%">Em submiss„o';
		$sx .= '<TH width="17%">Submetidos';
		$sx .= '<TH width="17%">Em an·lise';
		$sx .= '<TH width="17%">Para correÁıes';
	...
	function submit_resume_mostra_adms($page = '') {
		...
		$sx .= '<h1>Resumo das Submissıes</h2>';
		$sx .= '<TR>';
		$sx .= '<TH width="13%">Em submiss„o';
		$sx .= '<TH width="13%">Submetidos';
		$sx .= '<TH width="13%">Para revis„o do InglÍs';
		$sx .= '<TH width="13%">Para correÁıes do professor';
	...
	function cp_03() {
		...
		array_push($cp, array('$T80:4', 'sm_rem_01', 'IntroduÁ„o', True, True));
		array_push($cp, array('$T80:4', 'sm_rem_02', 'Objetivos', True, True));
		array_push($cp, array('$T80:4', 'sm_rem_03', 'MÈtodo', True, True));
		array_push($cp, array('$T80:4', 'sm_rem_04', 'Resultados (opcional para projetos)', False, True));
		array_push($cp, array('$T80:5', 'sm_rem_05', 'Conclus„o (opcional para projeto)', False, True));
	...
	function cp_05() {
		...
		array_push($cp, array('$DECLA', '', 'DeclaraÁ„o', True, True));
	...
	function comunitar_avaliador_externo_enviar($avaliador, $assunto, $texto) {
		...
		if (strlen($sql2) > 0) {
			...
			/* 17/30 */
			/* 08/11/2012 */
			$pa2 = array('PPGIa', 'ARQUR', 'MAT', 'CCOMP', 'FIS', 'ECV', 'ELE', 'MEC', 'EQ', 'ESAN', 'EPR', 'EBIO', 'PPGEM', 'PPGEPS', 'PPGTU', 'PPGIA', 'PPGTS', 'ADM', 'PLAN', 'DES', 'DIR', 'COMUM', 'COMUN', 'ARQU', 'TUR', 'PPAD', 'PPGD', 'PPGE', 'PPGF', 'PPGT', 'iPPGIa', 'iARQUR', 'iMAT', 'iCCOMP', 'iFIS', 'iECV', 'iELE', 'iMEC', 'iEQ', 'iESAN', 'iEPR', 'iEBIO', 'iADM', 'iPLAN', 'iDES', 'iDIR', 'iCOMUM', 'iCOMUN', 'iARQU', 'iTUR');

			/* 06/11/2012 */
			$pa1 = array('BIOG', 'BIOTEC', 'GEN', 'ZOO', 'ECO', 'MORF', 'FISIOL', 'BIOQ', 'BIOF', 'FARM', 'IMU', 'MICRO', 'PARA', 'MED', 'ODO', 'ENF', 'NUT', 'SC', 'FISIO', 'EF', 'BOTO', 'PPGCS', 'PPGO', 'AGRO', 'FLORE', 'MEDVET', 'CTA', 'PPGCA', 'EDU', 'PSICO', 'CPOLI', 'TEO', 'LING', 'LETR', 'ART', 'FILO', 'SOCIO', 'ANTRO', 'HIST', 'iBIOG', 'iBIOTEC', 'iGEN', 'iZOO', 'iECO', 'iMORF', 'iFISIOL', 'iBIOQ', 'iBIOF', 'iFARM', 'iIMU', 'iMICRO', 'iPARA', 'iMED', 'iODO', 'iENF', 'iNUT', 'iSC', 'iFISIO', 'iEF', 'iBOTO', 'iAGRO', 'iFLORE', 'iMEDVET', 'iCTA', 'iEDU', 'iPSICO', 'iCPOLI', 'iTEO', 'iLING', 'iLETR', 'iART', 'iFILO', 'iSOCIO', 'iANTRO', 'iHIST');
			$tt1 = '';
			$tt2 = '';
			$tt3 = '';
			$tt4 = '';
			...
			if (strlen($tt2) > 0) {$ttt .= $txt2 . '<BR>' . $tt2 . '<BR>';
			}
			if (strlen($tt3) > 0) {$ttt .= $txt3 . '<BR>' . $tt3 . '<BR>';
			}
			if (strlen($tt4) > 0) {$ttt .= $txt4 . '<BR>' . $tt4 . '<BR>';
			}
		...
		if (strlen($email) > 0) { enviaremail($email, '', $assunto, $texto);
			echo '<BR>enviado para ' . $email;
		}
		if (strlen($email1) > 0) { enviaremail($email1, '', $assunto, $texto);
			echo '<BR>enviado para ' . $email1;
		}
		return ('');
	...
	function comparacao_semic_pibic() {
		...
		while ($line = db_read($rlt)) {
			if (substr($line['pb_protocolo'], 0, 1) == '0') {
				...
				if (strlen($pb) == 0) { $cor = '<font color="red">';
					$err++;
				}
	...
	function lista_trabalhos_cicpg($id) {
		...
		while ($line = db_read($rlt)) {
			...
			if (strlen(trim($line['blk_data'])) > 0) { $local = $lc;
			}
	...
	function mostra_area($x) {
		$x = trim($x);
		$sx = $x;
		if ($x == 'A') { $sx = 'CiÍncias Exatas';
		}
		if ($x == 'B') { $sx = 'CiÍncias da Sa˙de';
		}
		if ($x == 'C') { $sx = 'CiÍncias Agr·rias';
		}
		if ($x == 'D') { $sx = 'CiÍncias Sociais Aplicadas';
		}
	...
	function lista_de_trabalhos_sql($todos = 0) {
		...
		if (strlen($dd[1]) > 0) { $modalidade = trim($dd[1]);
		}
		if (trim($dd[1]) == 'PIBIC') { $todos = 1;
		}
		...
		if ($todos != 1) { $sql .= " and article_modalidade = '$modalidade' ";
		}
		if (trim($dd[1]) == 'PIBIC') { $sql .= " and (article_modalidade = '$modalidade' or article_modalidade = 'IS' ) ";
		}
		if (strlen($internacional) > 0) { $sql .= " and article_internacional = '$internacional' ";
		}
	...
	function lista_de_trabalhos_to_word($todos = '') {
		...
		if ($todos == 'pos') {			$sql = "select * from articles
								left join sections on article_section = section_id					
								where articles.journal_id = $jid and article_publicado <> 'X' ";
	...
	function mostra_trabalho_linha($sql) {
		...
		while ($line = db_read($rlt)) {
			...
			if ($idx != $id) {
				...
				if ($xarea != $narea) {
					if ($moda == 'POS-G') { $moda = 'Doutorado / Mestrado';
					}
					$sx .= '<BR><BR>';
	...
	function projetos_sem_semic($ano) {
		...
		$sx .= '<TR><TD colspan=4>total de ' . $tot . ' resumos prontos para serem revisados em seu InglÍs';
		$sx .= '</table>';
		return ($sx);
	}
	...
	function reorganizar_trabalhos($ano) {
		...
		while ($line = db_read($rlt)) {
			...
			//if (round($line['article_revisado']) != 1)
			{
				...
				if ($mod == 'PIBIC_EM') { $ex = 'jr';
					$mod = 'PIBIC_EM';
				}
				if (substr(trim($line['abbrev']), 0, 2) == 'PP') { $ex = '';
					$mod = 'POS-G';
				}
	...
	function recupera_jid_do_semic($ano = 0) {
		...
		if ($ano == 0) { $semic = 'semic' . (date("Y") - 1992);
		} else { $semic = 'semic' . ($ano - 1992);
		}
		...
		if ($line = db_read($rlt)) {
			...
			//echo 'N„o foi localizado a path do journal  <b>'.$semic.'</B>';
			return (0);
			exit ;
		}
	...
	function resumos_sem_ingles($ano) {
		...
		$rlt = db_query($sql);
		$tot = 0;
		$sx .= '<H1>Resumos aguardando submiss„o do InglÍs</H1>';
		...
		$sx .= '<TR><TD colspan=2>total de ' . $tot . ' resumos prontos para serem revisados em seu InglÍs';
		$sx .= '</table>';
		return ($sx);
	}
	...
	function resumos_para_revisao($ano) {
		...
		$rlt = db_query($sql);
		$tot = 0;
		$sx .= '<H1>Resumos aguardando revis„o de InglÍs</H1>';
		$sx .= '<img src="../img/icone_star_2.jpg" height="24"> Prioridade na revis„o';
		$sx .= '<table width="100%" class="lt1" border=0 >';
		$sx .= '<TR><TH>Protocolo<TH width="25">en<TH>TÌtulo';
		$upx = '';
	...
	function area_resumo_gr($dd1 = '', $dd2 = '') {
		...
		if (strlen(trim($dd1)) > 0) { $sql .= " and (pb_ano = '" . $dd1 . "') ";
		}
		if (strlen(trim($dd2)) > 0) { $sql .= " and (pbt_edital = '" . $dd2 . "') ";
		}
		$sql .= ") as tabela ";
	...
	function area_resumo($tp = '') {
		...
		if (strlen(trim($dd[1])) > 0) { $sql .= " and (pb_ano = '" . $dd[1] . "') ";
		}
		if (strlen(trim($dd[2])) > 0) { $sql .= " and (pb_tipo = '" . $dd[2] . "') ";
		}
	...
	function area_detalhe() {
		...
		while ($line = db_read($rlt)) {
			...
			if ($idi != $idm) {
				if ($tot1 > 0) { $sx .= '<TR><TD colspan=4 align=right><I>subtotal ' . $tot1;
				}
				$tot1 = 0;
				$idi = $idm;
		...
		if ($tot > 0) { $sx .= '<TR><TD colspan=4 align=right><B><I>total ' . $tot;
		}
	...
	function idiomas_resumo() {
		...
		$si .= '<center>Idiomas de apresentaÁ„o</center>';
		...
		for ($r = 0; $r < count($data_idioma); $r++) {
			...
			$si .= $data_idioma[$r][0];
			$si .= '<TD align="center">';
			if ($toti > 0) { $si .= number_format(($data_idioma[$r][0]) / $toti * 100, 1) . '%';
			} else { $si .= '-';
			}
		}
		$si .= '</table>';
	...
	function idiomas_detalhe() {
		...
		if ($tote > 0) { $sx .= '<TR><TD colspan=4 align=right><I>subtotal em InglÍs ' . $tote;
		}
		if ($tot1 > 0) { $sx .= '<TR><TD colspan=4 align=right><I>subtotal ' . $tot1;
		}
		if ($tot > 0) { $sx .= '<TR><TD colspan=4 align=right><B><I>total ' . $tot;
	...
	function row() {
		...
		$cdm = array('', 'Codigo', 'Edital', 'InglÍs', 'Oral', 'Poster', 'Ano', '');
		$masc = array('', '', '', '', '', '', '', '');
		return (1);
	}
	...
	function cp() {
		...
		array_push($cp, array('$O  : &S:Sim&N:N„o', 'st_eng', 'InglÍs', True, True, ''));
		array_push($cp, array('$O  : &S:Sim&N:N„o', 'st_oral', 'Oral', FALSE, True, ''));
		array_push($cp, array('$O  : &S:Sim&N:N„o', 'st_poster', 'Poster', FALSE, True, ''));
