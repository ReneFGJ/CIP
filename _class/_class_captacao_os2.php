...
class captacao {
	...
	function captacao_listar($sta) {
		...
		switch ($sta) {
			case 1 :
				$sx .= '<h3>Para bonificar</h3>';
				break;
			case 2 :
				$sx .= '<h3>Sem bonificar e sem isenção</h3>';
				break;
			case 89 :
				$sx .= '<h3>Finalizado</h3>';
				break;
			case 80 :
				$sx .= '<h3>Em processo de bonificação</h3>';
				break;
			case 8 :
				$sx .= '<h3>Devolvido ao professor para correções</h3>';
				break;
			default :
	...
	function comunica_professor($tp = '', $texto) {
		...
		if (strlen($email_adm) > 0) { enviaremail($email_adm, '', $titulo, $text);
			echo 'enviado para ' . $email_adm;
		}
		if (strlen($email1) > 0) { enviaremail($email1, '', $titulo, $text);
			echo 'enviado para ' . $email1;
		}
...
function total_captacoes_validar($professor = '',$gestor=0) {
		...
		if ($id == 0) { $sx .= '<TR><TD colspan=10 class="lt4"><font color="red">Nenhum projeto para validar</font>';
		}
		$sx .= '</table>';
	...
	function status() {
		$ar = array('' => 'Não validado', '0' => 'Em cadastro', '80' => 'Em processo de bonificação', '81' => 'Bonificado', '99' => 'Não validado pelo coordenador', '98' => 'Validado, sem bonificação', '10' => 'Validado pelo professor', '1' => 'Com bonificação', '2' => 'Sem bonificação', '11' => 'Com bonificação e com isenção', '12' => 'Com bonificação e sem isenção', '13' => 'Sem bonificação e com isenção', '14' => 'Sem bonificação e sem isenção', '89' => 'Processo finalizado', '8' => 'Devolvido ao professor para correções', '9' => 'Cancelado');
		return ($ar);
	}
	...
	function captacao_agencia_ano($dd1 = 19000101, $dd2 = 20990101, $tp = 1, $detalhe = 0, $agencia = '') {
		...
		$anoi = substr($dd1, 0, 4);
		$anof = substr($dd2, 0, 4);
		if (strlen($agencia) > 0) { $where_age = " and (ca_agencia = '$agencia') ";
		}
	...
	function projetos_vigentes_inicio($dd1 = 19000101, $dd2 = 20990101, $tp = 1, $detalhe = 0, $agencia = '', $status = '', $programa = '', $institut = 0) {
		...
		if ($tp == 1) { $wh .= " and (ca_desmembramento <> '1' or ca_desmembramento isnull) ";
		}
		...
		while ($line = db_read($rlt)) {
			...
			if ($xproto != $proto) {
				...
				if ($line['ca_vlr_total'] > $line['ca_proponente_vlr']) { $cor = '<font color="blue">';
				} else { $cor = '';
				}
	...
	function cp_coordenador_editar() {/** Status **/
		$sss = $this -> status();
		$ops = ' 0:' . $sss[0];
		$ops .= '&1:' . $sss[1];
		$ops .= '&9:' . $sss[9];
	...
	function cp_01() {
		...
		array_push($cp, array('$R C:Proponente&P:Coordenador na Instituição&O:Colaborador&E:Coordenador no Programa/Escola', 'ca_participacao', $msg, True, True));
        ...
        $financiadorTipo = "SELECT * FROM agencia_de_fomento WHERE agf_ativo = 2 AND agf_sigla NOT LIKE 'EMP%' ORDER BY agf_ordem, agf_nome";
        
		array_push($cp, array('$A', '', 'Financiador', False, True));
		array_push($cp, array('$Q agf_nome:agf_codigo:'. $financiadorTipo, 'ca_agencia_gr', 'Financiador Tipo', True, True));
		array_push($cp, array('$Q agf_nome:agf_codigo:select * from agencia_de_fomento where agf_ativo = 1 order by agf_ordem, agf_nome', 'ca_agencia', 'Financiador', True, True));
	...
	function captacao_vigencia_bonificacao($anoi, $anof, $tipo = '') {
		...
		while ($line = db_read($rlt)) {
			...
			if (($void == 1) and (strlen(trim($line['ca_agencia_gr'])) == 0) or ($void == 0)) {
				...
				if ($tp == 'IPR') { $sa .= '<TD><IMG title="isenção de bolsa" SRC="../img/icone_academic.png" height="15">';
				}
				if ($tp == 'PRJ') { $sa .= '<TD><IMG title="bonificação 3%" SRC="../img/label_3percente_green.png" height="15">';
				}
			}
		}
		$sb .= '<TR><TD>' . $capx;
	...
	function captacao_ano_detalhe($ano = 2000, $programa = '') {
		...
		$sql = "select $cp from " . $this -> tabela . " 
					...
					and (ca_status = 98 or ca_status = 1 or ca_status = 80 or ca_status = 81)
					";
		if (strlen($programa) > 0) { $sql .= " and ca_programa = '$programa' ";
		}
		$sql .= "
					and ca_status <> 9
		...
		while ($line = db_read($rlt)) {
			...
			$prof = $line['pp_nome'];
			if ($prof != $pp) { $sx .= '<TR><TD colspan=10><B>' . $prof;
				$pp = $prof;
			}
	...
	function captacao_ano_agencia($ano = 2003, $agencia = '') {
		...
		$sql = "select $cp from " . $this -> tabela . " 
					inner join docentes on ca_professor = pp_cracha
					where ca_vigencia_ini_ano >= $ano and ca_participacao = 'C'";
		if (strlen($programa) > 0) { $sql .= " and ca_edital_codigo = '$agencia' ";
		}
		$sql .= "
					and (ca_status = 98 or ca_status = 1 or ca_status = 80 or ca_status = 81)
		...
		while ($line = db_read($rlt)) {
			...
			if ($tipo == 1) {
				if ($dd[1] == 'S') {
					...
					if (strlen($em1) > 0) { enviaremail($em1, '', 'Validação de captação', $ttt);
						echo '<BR>enviado para ' . $em1;
					}
					if (strlen($em2) > 0) { enviaremail($em2, '', 'Validação de captação', $ttt);
						echo '<BR>enviado para ' . $em2;
					}
				} else {
					echo $ttt;
					echo '<HR>';
	...
	function mostra_captacao_programas($dd1, $datai = 2000, $dataf = 2999) {
		...
		while ($line = db_read($rlt)) {
			...
			if ($xproto != $proto) {
				...
				if ($inst == '1') { $tot3 = $tot3 + $line['ca_proponente_vlr'];
					$tote++;
				} else {
					$tot7 = $tot7 + $line['ca_proponente_vlr'];
					$totf++;
					if ($tipo == 'C') { $tot = $tot + $line['ca_proponente_vlr'];
						$tot4 = $tot4 + $line['ca_vlr_total'];
						$totc++;
					}
					if ($tipo == 'O') { $tot2 = $tot2 + $line['ca_proponente_vlr'];
						$totd++;
					}
					if ($tipo == 'E') { $tot = $tot + $line['ca_proponente_vlr'];
						$tot4 = $tot4 + $line['ca_vlr_total'];
						$totc++;
					}
				}
	...
	function mostra_bonificados($dd1, $datai = 2000, $dataf = 2999,$tp='B') {
		...
		$sql = "select * from bonificacao
				...
				inner join pibic_professor on pp_cracha = pdce_docente
					where (pos_codigo = '" . $dd1 . "')
					$wh
					and bn_data >= $data1 and bn_data <= $data2
					and pdce_ativo = 1
					order by pp_nome
				";
		$rlt = db_query($sql);
...
//			$sx .= $line['bn_beneficiario'];
	...
	function mostra_captacao_programas_v2($dd1, $datai = 2000, $dataf = 2999) {
		...
		while ($line = db_read($rlt)) {
			...
			switch ($age) {
				case 'CNPq' :
					$finan[0] = $finan[0] + $valor;
					$qf[0] = $qf[0] + 1;
					break;
				case 'CAPES' :
					$finan[1] = $finan[1] + $valor;
					$qf[1] = $qf[1] + 1;
					break;
				case 'AG' :
					$finan[2] = $finan[2] + $valor;
					$qf[2] = $qf[2] + 1;
					break;
				case 'FA' :
					$finan[3] = $finan[3] + $valor;
					$qf[3] = $qf[3] + 1;
					break;
				case 'FINEP' :
					$finan[4] = $finan[4] + $valor;
					$qf[4] = $qf[4] + 1;
					break;
				case 'MCTI' :
					$finan[2] = $finan[2] + $valor;
					...
					break;
				case 'OPAS' :
					$finan[6] = $finan[6] + $valor;
					$qf[6] = $qf[6] + 1;
					break;
				case 'EMPRE' :
					$finan[7] = $finan[7] + $valor;
					$qf[7] = $qf[7] + 1;
					break;
				default :
					$finan[8] = $finan[8] + $valor;
					$qf[8] = $qf[8] + 1;
					break;
			}
	...
	function mostra_captacao_lista($line, $tipo = 0) {
		...
		if ($desmembramento == '1') { $cor = '<font color="#008080">';
		}
		if ($line['ca_status'] == 9) { $cor = '<font color="red">';
			$info = ' ** CANCELADO **';
		}
		...
		if ($ins == 1) {
			...
			if ($desmembramento == '1') { $sx .= '<TD>DESM.';
