			...
			if ($ac == '200')
				{
					...
					array_push($cp,array('$O : &1:Reabrir para postagem do relat�rio','','A��o',True,True));
			...
			if (($ac == '030') and ($dd[2]=='1'))
				{
					...
					$this->liberar_resubmissao_relatorio_parcial();
					$this->comunicar_pesquisador('Libera��o de rela�rio parcial',$historico);
			...
			if (($ac == '010') and ($dd[2]=='1'))	
				{
					...
					$this->comunicar_pesquisador('Suspens�o de bolsa',$historico);
					redirecina(page().'?dd0='.$dd[0].'&dd90='.$dd[90]);
					exit;
				}
	...
	function semic_area_de_apresentacao($ano=0,$mod='')
		{
			...
			$sx .= '<TR><Th width="10%">Total<TH width="10%">CNPq
					<TH width="80%" align="left">Descri��o da �rea';
	...
	function acompanhamento_avaliacao_relatorio_parcial()
		{
			...
			while ($line = db_read($rlt))
				...
				$sx .= '<TR><TH>Modalidade<TH>� entregue<TH>Entregue<TH>Gr�fico';
				$sx .= '<TH>Percentual';
	...
	function acompanhamento_avaliacao_relatorio_aprovacao()
		{
			...
			while ($line = db_read($rlt))
				...
				$sx .= '<TR><TH>Modalidade<TH>aprovado<TH>� Aprovado<TH>Gr�fico';
    ...
    <script type="text/javascript">
	  ...
	  $sx .= '
        ...
        new google.visualization.ColumnChart(document.getElementById(\'visualization\')).
            draw(data,
                 {title:"Titula��o dos pesquisadores em Inicia��o Cient�fica",
		...
		function rel_curso_bolsas($ano)
			{
				...
				echo 'Legenda: PIBIC - Inicia��o Cient�fica; PIBITI - Inicia��o Tecnol�gica; PIBIC_EM - Inicia��o J�nior (Ens. M�dio); CSF - Ci�ncia sem fronteiras';
		...
		function mostra_relatorios_status()
			{
				...
				$ns = array('-','aprovado','n�o aprovado','nao avaliado','','','','','','-','');
