...
//				array_push($cp,array('$}','','Dados do projeto do professor',False,True));
	...
	function resumo_planos_area_conhecimento($ano='2013',$edital = '',$status='B')
		{
			...
			$areas = array('Ciências Exatas e da Terra',
								'Ciências Biológicas ',
								'Engenharias',
								'Ciências da Saúde',
								'Ciências Agrárias',
	...
	function valida_post()
		{
			...
			if (strlen(trim($dd[20])) ==0 ) 
				{ $valid = 0; array_push($err,'Área do conhecimento não informada'); }
	...
	function valida_arquivo($tipo='',$protocolo='')
		...
		function mostra_plano_botao($nr)
			{
				if ($nr==1 or $nr==2)
					{ $sx = $this->mostra_botao_submissao('IC'); }
				if ($nr == 3)
					{ $sx = $this->mostra_botao_submissao('ICI'); }
				if ($nr == 4)
					{ $sx = $this->mostra_botao_submissao('JR'); }
				return($sx);
			}		
		...
		function mostra($line,$id='',$edit=1)
			{
				global $user;
					...
					switch ($sta)
						{
						case '@': $sx .= '<font color="orange"><B>Em submissão</B><font>'; break;
						case '!': $sx .= '<font color="orange"><B>Em submissão</B><font>'; break;
						case 'A': $sx .= '<font color="white"><B>Submetido em '.stodbr($line['pj_data']).'</B><font>'; break;
						case 'B': $sx .= '<font color="white"><B>Submetido em '.stodbr($line['pj_data']).'</B><font>'; break;
						case 'E': $sx .= '<font color="white"><B>Avaliação Finalizada</B><font>'; break;
