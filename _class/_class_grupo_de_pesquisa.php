	...
	function mostra_dados()
		{
			...
			if (strlen($gr_link_cnpq) > 0)
				{ $gr_link_cnpq = '<A HREF="'.$gr_link_cnpq.'" target="new">'.$gr_link_cnpq.'</A>'; }
			...
			$sx .= "<fieldset><legend>".msg('group_research')."</legend>
					...
					<TR class=lt4 ><TD colspan=5><B>$group_name</TD>
					<TR><TD>
	...
	function mostra_lider()
		{
			...
			/*** Sem lider **/
			if ($tot == 0)
				{ $sx .= '<TR><TD colspan=4 align=center class=lt2><font color=red>'.msg('not_leader').'</font></TD>'; }
			$sx .= '</table></fieldset>';
			return($sx);
		}
	function update()
	...
	function msg($ac)
		{
			switch($ac)
				{
				case 'action_A': $ac = 'Incluir novo lider'; break;
				case 'action_B': $ac = 'Incluir novo membro'; break;
				case 'action_C': $ac = 'Excluir membro ou lider'; break;
				}
			return($ac);
		}
	...
	function grupo_lattes_status()
		{
			$st = array(
				-1=>'Não identificado',
				1=>'processo de carga',
				2=>'certificado pela instituição',
				3=>'certificado pela instituição - Não atualizado há mais de 12 meses',
				4=>'aguardando certificação pela instituição',
				5=>'em preenchimento'
				);
			return($st);
		}
