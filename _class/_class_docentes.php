	...
	function inport_titulacao($c4='')
		{
			...
			if ($c4a == '000')
				{ echo '<BR><font color="red">Não localizado titulação ('.$c4.') - função inport_titulacao(); </font>'; exit; }
			return($c4a);
		}
	...
	function mostra_ano($v)
		{
			...
			if (($v==0) or ($v==1900)) { $v = '-'; } else { $v = strzero($v,4); }
			return($v);
		}
	...
	function status()
		{
			...
			$sta = array('A'=>'Ativo (cursando)' ,'Q'=>'Qualificado',
			'C'=>'Ativo (cursando)', 
			'O'=>'Requalificado',
			'D'=>'Defendido',
			'T'=>'Titulado',
	...
	function higienizacao_curso()
		{				
			...
			while ($line = db_read($rlt))
			{
				...
				if (strpos($curso,'-') > 0) { $curso_novo = substr($curso,0,strpos($curso,'-')); echo '<HR>'.$curso_novo.'<hr>'; }
	...
	function mostra()
		{
			...
			$lattes = troca($lattes,'.jsp','.do');
			if (strlen($lattes) > 0)
				{ $lattes_img = '<a href="'.$lattes.'" target="new"><img src="'.http.'/img/logolattes.gif" border=0></A>'; }
			if (strlen($lattes) > 0)
				{ $lattes = '<a href="'.$lattes.'" target="new">'.$lattes."</A>"; }
			$img_photo = $this->recupera_foto();
