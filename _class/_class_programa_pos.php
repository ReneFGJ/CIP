...
';
	...
	function programas_pos_cronologico() {
		...
		while ($line = db_read($rlt)) {
			...
			$sx .= '<TR><TD align="left" class="tabela01">' . $line['pos_nome'];
			if ($anof <> date("Y")) { $sx .= '<font color="red">(Encerrado)</font>';
				$ide++;
			}
...
';
	...
	function programas() {
		...
		while ($line = db_read($rlt)) {
			...
			$sx .= '<TD class="tabela01" align="center">';
			if ($ano_mestrado > 0) { $sx .= $ano_mestrado;
			} else { $sx .= '-';
			}

			$sx .= '<TD class="tabela01" align="center">';
			if ($ano_doutorado > 0) { $sx .= $ano_doutorado;
			} else { $sx .= '-';
			}
			$ln = $line;

			$sx .= '<TD class="tabela01" align="center">';
			if ($ano_mestrado_prof > 0) { $sx .= $ano_mestrado_prof;
			} else { $sx .= '-';
			}
	...
	function mostra_docentes_orientacoes_detalhe($docente, $programa) {
		...
		if (strlen($ms) > 0) { $sx1 .= '<TR><TD colspan=6><B>Mestrado</B>' . $ms;
		}
		if (strlen($dr) > 0) { $sx1 .= '<TR><TD colspan=6><B>Doutorado</B>' . $dr;
		}
		if (strlen($po) > 0) { $sx1 .= '<TR><TD colspan=6><B>Pós-Doutorados</B>' . $po;
		}
	...
	function mostra() {
		...
		if ($mestrado > 0) { $modalidade .= 'M';
			$nts = 'MS';
		}
		if ($doutorado > 0) { $modalidade .= '/D';
			$nts = 'DR';
		}
	...
	function produtividade() {
		$pd = array(0 => ' ', 1 => '--', 2 => 'Nível PQ 1A', 3 => 'Nível PQ 1B', 4 => 'Nível PQ 1C', 5 => 'Nível PQ 1D', 6 => 'Nível PQ 2', 12 => 'Nível DT 1A', 13 => 'Nível DT 1B', 14 => 'Nível DT 1C', 15 => 'Nível DT 1D', 16 => 'Nível DT 2');
		return ($pd);
	}
