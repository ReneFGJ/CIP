...
if ($busca ==  true)
	...
	/* Formulário */
		...
		echo '<TD width="5"><NOBR>'.msg('search').'&nbsp;</TD>';
		echo '<TD width="5">';
		echo '<input type="text" name="dd1" value="'.$dd[1].'" class="input_01" ';
		echo 'size="15" maxlength="80" >';
	
	/* Botão Pesquisar */
		echo '<TD width="95">';
		echo '<a class="bt" href="#" title="search" onclick="document.row.submit();">'.msg('filter').'</a>';
		echo '<input type="hidden" name="dd50" value="filtrar">';

	/* Pesquisar em */	
		echo '<TD width="5"><NOBR>&nbsp;em&nbsp;</TD>';
		echo '<TD width="5"><select name="dd2" size="1" class="IMG"  class="input_01">';
		echo $cp_ed;
		echo '</select>';
		echo '<TD>&nbsp;&nbsp;';
	...
	/* Botão Limpar */
		...
		echo '<a class="bt" href="#" title="clear" onclick="document.row.dd50.value='.chr(39).'clean'.chr(39).'; document.row.submit();">'.msg('clear_filter').'</a>';
		echo '<img src="'.$include.'img/bt_alert.jpg" title="Atention! Filter Active!'.chr(13).msg('filter_ce').'" height=16>';
		}
		
	/* Seleção mostrar */
		echo '<TD width="5" align="right">'.msg('show_records').'&nbsp;</form></TD>';
	
	/* $http_redirect.'?dd1='.$dd[1].'&dd4='. */
		echo '<TD width="5">';
		echo '<select name="dd4" size="1" class="IMG"  class="input_01" ';
	...
	if ($editar) 
		{ 
		echo '<TD align="right">';
		echo '<a class="bt" href="#" title="search" onclick="location.href=\''.$http_edit.$http_ch.$http_edit_para.'\'">'.msg('new_record').'</a>';
		}
