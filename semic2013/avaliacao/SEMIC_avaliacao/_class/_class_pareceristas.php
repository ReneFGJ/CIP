		...
		function row()
			{
				...
				$tabela = "(select * from pareceristas inner join instituicao on us_instituicao = inst_codigo ) as tabela ";
				$cdf = array('id_us','us_nome','inst_abreviatura','us_codigo');
				$cdm = array('cod',msg('nome'),msg('instituicao'),msg('codigo'));
				$masc = array('','','','','','','');
				return(1);				
			}
				...
				if (strlen($area) > 0)
						{
							...
							$title = 'Avaliador Iniciação Científica - PIBIC/PIBITI';
							enviaremail('pibicpr@pucpr.br','',$title.' (copia)',$txt);
							enviaremail('renefgj@gmail.com','',$title.' (fim)',$txt);
		...
		function lista_professores_ic()
			{
				...
				$rlt = db_query($sql);
				$sx = '<table width="100%">';
				$sx .= '<H2>Professores Doutores/IC</h2>';
		...
		function cp()
			{
				...
				//array_push($cp,array('$Q inst_nome:inst_codigo:select * from instituicoes order by inst_ordem, inst_nome','us_instituicao','Instituição',True,True,''));
				...
				array_push($cp,array('$O -1:Não informado&10:SIM, Convite Novo Aceito&1:Limbo de aceite&0:NÃO&9:Enviar convite&2:Aguardando aceite do convite&3:inativo temporariamente','us_aceito','Aceito com parecerista',False,True,''));
				...
				if (strlen(trim($this->email)) > 0)
					{
						...
						enviaremail('renefgj@gmail.com','','Atualização de dados',$body);
						echo $email.' ';
					}
...
/* */																		
		...
		function mostra_link_avaliador($link)
			{
				...
				if (strlen(trim($link)) > 0)
					{
					$img_link = '<img src="img/icone_link.png" height="10">';
		...
		function mostra_dados()
			{
				...
				$sx = '';
				$sx .= '<!-- TOP INTERNAL CONTENT-->
						<table style="width: 370px; float: left;" border=0>
							<TR valign="top"><TD>
		...
		function login_set($id='')
			{
				...
				if ($line = db_read($rlt))
				{
					...
					enviaremail('renefgj@gmail.com','','Erro de acesso','Erro de acesso '.$dd[0].'=='.$dd[90]);
					exit;
				}
				return(1);
			}
		...
		function ASC_update()
			{
				$sql = "select * from ".$this->tabela." where (us_nome_asc = '' and us_nome <> '') ";
				$rlt = db_query($sql);
				while ($line = db_read($rlt))
				{
