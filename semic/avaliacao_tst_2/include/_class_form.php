...
class form
	{
		...
		var $class='';
		var $class_memo;
		/* Oculta mensagem do campo obrigatorio */
		var $hidden_error = 0;
		/* Oculta mensagens de campos obrigatorios apos um submit */
		var $hidden_form_error = 1;
		...
		function editar($cp,$tabela,$post='')
			{
				...
				$this->saved = 1;
				$this->rq = '';
				$sx .= '<form id="formulario" method="post" action="'.$post.'">'.'<table class="tabela00" width="100%">'.chr(13);
				...
				if ((strlen($this->rq) > 0) and (strlen($acao) > 0) and ($this->hidden_form_error==0))
					{
		...
		function process($cp)
			{
				...
				if ((strlen($acao) > 0) and ($this->required==1) and (strlen($this->value)==0) and ($this->hidden_error==0))
		...
		function requerido()
			{
				...
				if ($this->required == 1)
					{
						if ((strlen($this->value) == 0 ) and ($this->hidden_error == 0))
						{ 
