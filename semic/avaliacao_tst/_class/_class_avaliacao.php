...
class Avaliacao
	...
	function insere_avaliacao_forcado($idAvaliadorUnsafe, $idTrabalhoStrUnsafe, $tipoTrabalhoUnsafe)
		{
			$tipoTrabalhoUnsafe = trim($tipoTrabalhoUnsafe);
			$tipoTrabalhoUnsafe = trim(substr($tipoTrabalhoUnsafe,0,1));
			$tipoTrabalhoAbrev = $tipoTrabalhoUnsafe;
			
			if ($tipoTrabalhoUnsafe == 'P') { $tipoTrabalhoUnsafe = 'Poster'; }
			if ($tipoTrabalhoUnsafe == 'O') { $tipoTrabalhoUnsafe = 'Oral'; }
			$num = sonumero($idTrabalhoStrUnsafe);
			$area = substr($idTrabalhoStrUnsafe,0,strpos($idTrabalhoStrUnsafe,$num));
			$num = substr($idTrabalhoStrUnsafe,strlen($area),20);
			
			//$sql = "alter table pibic_semic_avaliador add column psa_manual integer default 0";
			//$rlt = db_query($sql);
			$tipo = $this->eventos[0][0];
			
			if (substr($area,0,3) == 'PPG') { $tipo = $this->eventos[1][0]; }
			$sqlx = "select * from pibic_semic_avaliador
					where psa_p01 = '$idAvaliadorUnsafe' 
					and psa_p02 = '$num'
					and psa_p03 like '$tipoTrabalhoAbrev%'
					and psa_p04 = '$area' ";
			$rlt = db_query($sqlx);
			while ($line = db_read($rlt))
				{
					$ref = trim($line['psa_p04']).trim($line['psa_p02']);
					redirecina('semic_exibe_resumo.php?dd51='.$ref);
				}

			$sql = "insert into pibic_semic_avaliador 
					(psa_p01, psa_p02, psa_p03,
					 psa_p04, psa_p05, 
					 
					 psa_abe_1, psa_abe_2, psa_abe_3,					 
					 psa_abe_4, psa_abe_5, psa_final,
					 psa_checked, psa_manual,
					 
					 psa_nt_1, psa_nt_2, psa_nt_3,					 
					 psa_nt_4, psa_nt_5, psa_nt_6,
					 psa_nt_7, psa_nt_8, psa_nt_9
					 ) values (
					 '$idAvaliadorUnsafe','$num','$tipoTrabalhoUnsafe',
					 '$area','$tipo',
	...
	function cp_id_trabalho_manual()
		{
			...
			if (strlen($dd[1]) > 0)
				{
				array_push($cp,array('$M8','','<span style="color: red; text-align:right; font-weight:normal;"><i>Código de trabalho incorreto ou não publicado.</i></span>',False,True));
				}
