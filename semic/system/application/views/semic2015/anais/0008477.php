<div id="trabalho">
	<table width="100%" border=0>
		<tr valign="top">
			<td colspan=2>LIMA, I. C. M.; KUMODE, M. M. N. Caracterização física e mecânica dos compósitos elaborados com casca de pinhão em matriz  termofixa. In: SEMIC, Seminário de Iniciação Científica, 23, 2015, Curitiba-PR. Anais do 23º Seminário de Iniciação Científica. Curitiba: PUCPR, 2015. p. FLORE27.</td>
			<td width="100" align="right" rowspan=2>
				Pôster				<br>
				<img src="<?php echo base_url('img/semic/icone-poster-grad.png');?>" title="Pôster">
			</td>
		</tr>
		<tr>
			<td colspan=2 class="lt3"><font class="lt6">FLORE27</font>
				<br><b>Recursos Florestais e Engenharia Florestal</b></td>
		</tr>
		<tr>
			<td align="center"><font class="lt5"><b>Caracterização física e mecânica dos compósitos elaborados com casca de pinhão em matriz  termofixa</b></font>
			<BR>
			<font class="lt4"><i>Physicals and mechanicals characteristics of thermosetting composites prepared with pine nuts.</i></font>
			<BR>
			</td>
		</tr>
	</table>
	<br>
	<div style="text-align:right;">
		LIMA, Isabelle Caroline Martins<sup>1</sup>; KUMODE, Marina Mieko Nishidate<sup>2</sup>		<BR>
		PIBIC		- Opção para Iniciação Científica Voluntária		<BR>
		<I>--</I>
	</div>
	<BR>
	<table width="100%">
		<tr valign="top">
			<td width="44%"> Resumo
			<div style="text-align:justify;">
				<P>
					<B>Introdução</B>: O pinhão é um produto bastante promissor economicamente, porém, perde-se muito de sua biomassa, uma vez que toneladas de casca de pinhão são jogadas no lixo anualmente.					<B>Objetivo</B>: O presente trabalho visa demonstrar a viabilidade de utilizar a casca de pinhão como
matéria prima para desenvolvimento de compósitos termofixos utilizando Uréia Formaldeído (UF) e Fenol Formaldeído (FF), na concentração de
20% e 30%.					<B>Metodologia</B>: Para a confecção dos compósitos, as cascas foram previamente secas em estufa, moídas e classificadas granulométricamente na peneira de 42 mesh e retida na de 60 mesh. Para a homogeneização das resinas com a casca, utilizou-se um homogeneizador termocinético a 1500 rpm por 30 s, seguidos de 3000
rpm por 30 s. Em seguida , o material foi prensado a 120±2°C por 5 min
com pressão de 4 MPa em prensa hidráulica Marconi MA098/A. Para os
 testes de flexão estática utilizou-se uma máquina de ensaio universalmarca EMIC, seguindo a norma (ISO 178), realizando 10 repetições. Os testes de absorção de água e inchamento foram realizados utilizando a norma (ASTM - 570-98), com 5 repetições. Para a massa específica, utilizou-se a norma (ASTM – D 792-91). As caracterizações morfológicas foram realizadas por microscopia eletrônica de varredura.					<B>Resultados</B>: As resistências à flexão estática aumentaram 14% e 89% com o incremento no teor de resinas UF e FF, respectivamente. Para baixos teores de resina os compósitos com UF foram mecanicamente mais resistentes. Quanto a absorção de água e inchamento, os compósitos de UF foram os mais higroscópicos.					<B>Conclusões</B>: Com o aumento da resina, a performance dos compósitos melhorou quanto a resistência a umidade  as resistências mecânicas. Portanto, é possível utilizar a casca de pinhão
 para confecção de compósitos termofixos.				</P>
			</div><B>Palavras-chave</B>: casca de pinhão, compósito, caracterização.			<BR>
			<BR>
			</td><td width="2%">&nbsp;</td><td width="44%"> Abstract
			<div style="text-align:justify;">
				<P>
					<B>Introduction</B>: The pine nuts is a very promising product; however, large tons of husk of pine nuts are thrown away annually.					<B>Objectives</B>: This study aims to demonstrate aims the viability of using husk of pine nuts for development of composites in thermoset matrix using Urea Formaldehyde (UF) and Phenol Formaldehyde (PF), on the concentration of 20% and 30%.					<B>Methods</B>: For the manufacturing of the composites, the husk of pine nuts were previously dried, grounded and classified granulometrically on the sieve of 42 mesh and retained on 60 mesh. For the mixing of the materials, a thermos-kinetic homogenizer was used at 1500 rpm for 30 s,
followed by 3000 rpm for 30 s. After, the mixture was molded by flatpressing, using a Marconi MA098/A hydraulic press. For bending tests, it was used an EMIC universal tests machine, following the (ISO 178), performing 10 replicates. Water absorption and thickness swelling tests were conducted following the (ASTM D792-08). with 5replicates. The morphological characterizations were performed by scanning electron
microscopy.					<B>Results</B>: gth increased 14% and 89% withthe increase of the resins. At low resin contents, UF composites were mechanically more resistant. About the water absorption and thickness
swelling, UF composites were more hygroscopic.					<B>Conclusion</B>: The increase in the proportion of the resins improved the performance of composites for moisture resistance. Therefore, it is possible to use the husk of pine nuts for making thermoset composites, however, their use should be for purposes where the need for large mechanical resistances are low.				</P>
			</div><B>Keywords</B>: pinis nuts, composites, characterization.</td>
		</tr>
	</table>
	<BR>
	<BR>
	<sup>1</sup> Estudante. PUCPR<br><sup>2</sup> Orientador. PUCPR	<BR>
	<BR>
	<BR>
	<BR>
</div>
<BR>
<BR>
<BR>
</div> 