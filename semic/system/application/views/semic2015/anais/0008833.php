<div id="trabalho">
	<table width="100%" border=0>
		<tr valign="top">
			<td colspan=2>IRASSOQUE, G. F.; GUIMARAES, B. S. Competência do serviço público de saneamento básico na região metropolitana. In: SEMIC, Seminário de Iniciação Científica, 23, 2015, Curitiba-PR. Anais do 23º Seminário de Iniciação Científica. Curitiba: PUCPR, 2015. p. DIRpb4.</td>
			<td width="100" align="right" rowspan=2>
				Oral				<br>
				<img src="<?php echo base_url('img/semic/icone-oral-grad.png');?>" title="Oral">
			</td>
		</tr>
		<tr>
			<td colspan=2 class="lt3"><font class="lt6">DIRpb4</font>
				<br><b>Direito Público</b></td>
		</tr>
		<tr>
			<td align="center"><font class="lt5"><b>Competência do serviço público de saneamento básico na região metropolitana</b></font>
			<BR>
			<font class="lt4"><i>Competence of sanitation public service in the metropolitan area</i></font>
			<BR>
			</td>
		</tr>
	</table>
	<br>
	<div style="text-align:right;">
		IRASSOQUE, Geremias Fernandes<sup>1</sup>; GUIMARAES, Bernardo Strobel<sup>2</sup>		<BR>
		PIBIC		- Bolsa PUCPR-PIBIC		<BR>
		<I>--</I>
	</div>
	<BR>
	<table width="100%">
		<tr valign="top">
			<td width="44%"> Resumo
			<div style="text-align:justify;">
				<P>
					<B>Introdução</B>: Uma região metropolitana ao ser instituída tem como finalidade a viabilização, a organização, a execução e o planejamento das funções públicas de interesse
comum a todos os seus membros. Uma vez configurada a regionalização, não se trata mais de um sistema de saneamento básico isolado, mas de um sistema de saneamento básico integralizado. Os serviços públicos de saneamento básico estão diretamente relacionados com a saúde pública. Esta, por sua vez, é parte de um conjunto de direitos sociais, que têm como inspiração o valor da igualdade entre as pessoas. Por isso, o presente propósito acadêmico tem por objetivo responder de modo mediato à seguinte questão: a quem compete o serviço público de saneamento básico em regiões metropolitanas? Por outro lado, o objetivo geral se desdobra em dois objetivos específicos no que se refere ao serviço de saneamento básico. São eles: (i) investigar os referenciais normativos, a doutrina e a jurisprudência do Supremo Tribunal Federal aplicáveis à questão, (ii) avaliar a titularidade dos serviços de saneamento básico em regiões metropolitanas.					<B>Objetivo</B>: O presente propósito acadêmico tem por objetivo responder de modo
mediato à seguinte questão: a quem compete o serviço público de saneamento
básico em regiões metropolitanas?
O objetivo geral se desdobra em dois objetivos específicos no que se refere
ao serviço de saneamento básico. São eles: (i) investigar os referenciais normativos,
a doutrina e a jurisprudência do Supremo Tribunal Federal aplicáveis à questão,
avaliando se o modelo implementado de prestação de serviço público de
saneamento básico atende às diretrizes de proteção dos interesses dos usuários e
(ii) avaliar a titularidade dos serviços de saneamento básico em regiões
metropolitanas, de modo a enxergar como as regras jurídicas podem servir de
elemento para gerar a necessidade de convergência de interesses entre os diversos
entes envolvidos.					<B>Metodologia</B>: O método a ser utilizado foca na consulta às seguintes fontes: referenciais normativos que se aplicam à questão a ser examinada; revisão doutrinária acerca do tema e, por fim,
análise de jurisprudência sobre o assunto.					<B>Resultados</B>: 					<B>Conclusões</B>: 				</P>
			</div><B>Palavras-chave</B>: Serviço público, saneamento básico			<BR>
			<BR>
			</td><td width="2%">&nbsp;</td><td width="44%"> Abstract
			<div style="text-align:justify;">
				<P>
					<B>Introduction</B>: A metropolitan area to be established aims at enabling the organization, execution and planning of public functions of interest common to all its members. Once set regionalization, it is no longer an isolated sewerage system, but a paid sanitary sewer system. Public sanitation services are directly related to public health. This, in turn, is part of a set of social rights, whose inspiration the value of equality between people. Therefore, the academic purpose this aims to respond mediate way the question: who is responsible for the public service of sewerage in metropolitan areas? On the other hand, the general objective unfolds in two specific objectives with regard to sanitation services. They are: (i) investigate the normative references, the doctrine and jurisprudence of the Supreme Court apply to the question, (ii) evaluate the ownership of the sanitation services in metropolitan areas.					<B>Objectives</B>: The academic purpose of this is aimed at answering mode mediate the following question: who is responsible for the sanitation of public service basics in metropolitan areas? The overall objective unfolds in two specific objectives with regard the sanitation service. They are: (i) investigate the normative references, the doctrine and jurisprudence of the Supreme Court apply to the question, evaluating the model implemented to provide public service sanitation suits protection guidelines of the interests of users and (ii) evaluate the ownership of the sanitation services in regions metropolitan in order to see how the legal rules can serve as element to generate the need for convergence of interests between the various entities involved.					<B>Methods</B>: The method being used focuses on consulting the following sources: regulatory frameworks that apply to the question to be examined; doctrine review on the subject and, finally, jurisprudence analysis on the subject.					<B>Results</B>: 					<B>Conclusion</B>: 				</P>
			</div><B>Keywords</B>: Public service, sanitation</td>
		</tr>
	</table>
	<BR>
	<BR>
	<sup>1</sup> Estudante. PUCPR<br><sup>2</sup> Orientador. PUCPR	<BR>
	<BR>
	<BR>
	<BR>
</div>
<BR>
<BR>
<BR>
</div> 