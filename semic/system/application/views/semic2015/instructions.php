<h1>NORMAS GERAIS PARA PARTICIPAÇÃO NO XXIII SEMIC</h1>
<br />
<br>
<br>
<a href="#" onclick="newwin2('https://cip.pucpr.br/index.php/semic/poster_localizacao/',800,600);">Localize seu pôster aqui!</a>
<script>
	function newwin2(url) {  NewWindow=window.open(url,'newwin','scrollbars=yes,resizable=yes,width=850,height=650,top=10,left=10');  NewWindow.focus(); void(0);}
</script>
</br>
<h2>Orientações</h2>
<div align="left" class="content-6">
	<p align="justify">
		Para localizar o código do trabalho, o aluno deverá acessar o site do XXIII SEMIC (www.pucpr.br/semic) entrar no link em Sumário Geral, apertar CTRL + F, abrirá uma caixa de "localizar", deverá ser digitado o sobrenome do autor em maiúsculo e apertar Enter, automaticamente será remitido ao projeto com informações de data, local e horário de apresentação. O aluno deverá anotar este código, pois ele será o identificador do seu trabalho.
	</p>
	<p align="justify">
		Para as apresentações em inglês os códigos são identificados com letras "i" na frente.
	</p>
	<p align="justify">
		Para as apresentações dos trabalhos PIBITI todos são identificados com a letra "T" ao final do código.
	</p>
	<p align="justify">
	<p>
		Exemplo:
		<br/>
		<ul>
			<li>
				<strong>INTERNACIONAL:</strong>
				<ul>
					<li>
						<b>iEPR07</b> (trabalho de engenharia de produção, em inglês, número 07)
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>PIBIC:</strong>
				<ul>
					<li>
						<b>CCOMP44</b> (trabalho PIBIC de ciência da computação, número 44)
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>PIBITI: </strong>
				<ul>
					<li>
						<b>EPR08T</b> (trabalho PIBITI de engenharia de produção, número 08)
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>PIBIC Jr:</strong>
				<ul>
					<li>
						<b>medvet01Jr</b> (trabalho PIBIC JUNIOR de medicina veterinária, número 01)
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>MOBILIDADE NACIONAL:</strong>
				<ul>
					<li>
						<b>MOB02</b> (trabalho PIBIC ou PIBITI Mobilidade Nacional, número 02)
					</li>
				</ul>
			</li>
		</ul>
	</p>
	<br />
	<p align="justify">
		O aluno deverá participar de todo o bloco em que seu trabalho está incluído, por exemplo: das 8h00 às 09h15. Não será permitida a entrada do aluno na sala após o início das apresentações ou apenas para a sua apresentação. O aluno deverá utilizar uma apresentação de Power Point (modelo encontrado no site do <a href="http://www.pucpr.br/pesquisacientifica/iniciacaocientifica/modelosdedocumentos.php" target="_new">PIBIC</a>) como material de apoio.
	</p>
	<p align="justify">
		Para sua segurança envie uma cópia para o seu e-mail. Assim evitará problemas de arquivo corrompido na hora da apresentação. Não é necessário entregar para a Coordenação antecipadamente.
	</p>
	<p align="justify">
		Os alunos que optaram pela apresentação oral internacional, deverão fazer os slides em inglês, seguindo as mesmas normas acima.
	</p>
	<p align="justify">
		A apresentação oral deverá ter no <font color = "#8b1217">máximo 10 minutos</font>, mais <font color = "#8b1217">05 minutos de perguntas e respostas</font> para uma banca de dois avaliadores.
	</p>
	<p align="justify">
		A equipe de apoio do evento está terminantemente proibida de auxiliar o aluno na troca de slides no momento da apresentação e na instalação da apresentação no computador da sala de apresentação. Assim, o aluno deverá pedir a um colega que o auxilie.
	</p>
	<p align="justify">
		Para fazer a transferência de arquivo para o computador da sala, é necessária a chegada antecipada (no intervalo de apresentações), uma vez que a partir da hora que o avaliador chamar o aluno para apresentar, o tempo já estará contando.
	</p>
	<br />
	<p>
		<strong>PRESENÇA</strong>
	</p>
	<p align="justify">
		É obrigatória a presença do orientador durante a apresentação oral do aluno. Caso ele esteja impossibilitado de comparecer, é necessário que envie um representante com um ofício de justificativa da ausência. Este ofício deverá ser entregue para o avaliador que estiver presidindo a mesa, bem como enviado uma cópia para a Coordenação da IC.
	</p>
	<br />
	<p>
		<strong>PARTICIPAÇÃO</strong>
	</p>
	<p align="justify">
		O aluno que quiser participar como ouvinte deverá assistir obrigatoriamente pelo menos 50% das apresentações oral, palestras, minicurso e/ou mesa redonda.
		*** Sessões de pôster NÃO contabilizam como frequência para completar os 50% de participação no evento.
	<p align="justify">
		Alunos que apresentarão seus trabalhos no evento receberão a declaração de conclusão e participação no programa PIBIC/PIBITI.
	</p>
	</p>
	<br />
	<p>
		<strong>DECLARAÇÃO</strong>
	</p>
	<p align="justify">
		O certificado será disponibilizado no site do evento a partir do dia 25/10/2015, e poderá ser obtido no painel do usuário. Somente usuários com credenciamento efetivo terão direito ao certificado de participação, portanto todos os alunos deverão obrigatoriamente efetuar o credenciamento a partir do dia 05/10/2015 conforme instruções.
	</p>
	<br />
	<h1>Sessão de Pôster</h1>
	<p align="justify">
	<p>
		<strong>Sessões:</strong>
		<br/>
		<ul>
			<li>
				<strong>Sessão 1 - dia 06/10 das 10h30 às 12h30 </strong>
				<ul>
					<li>
						PIBIC (Ciências Sociais Aplicadas e Ciências Agrárias).
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>Sessão 2 - dia 06/10 das 15h30 às 17h30 </strong>
				<ul>
					<li>
						PIBIC (Ciências Exatas e Ciências sem Fronteiras/Intercâmbio);
					</li>
					<li>
						PIBITI (Todas as áreas).
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>Sessão 3 - dia 07/10 das 10h30 às 12h30 </strong>
				<ul>
					<li>
						PIBIC (Ciências Humanas);
					</li>
					<li>
						PIBIC JR (todas as áreas).
					</li>
				</ul>
			</li>
			<br />
			<li>
				<strong>Sessão 4 - dia 07/10 das 15hh30 às 17h30</strong>
				<ul>
					<li>
						PIBIC (Ciências da Vida).
					</li>
				</ul>
			</li>
			<br />
		</ul>
	</p>
	<p align="justify">
		Para apresentação do pôster o aluno deverá fixa-lo momentos antes da sessão iniciar e permanecer ao lado do banner durante toda a sessão, ou até realizar a sua apresentação, após a apresentação o aluno deverá retirar o pôster  do local fixado. A sessão tem duração de 2 horas, portanto, o aluno deverá aguardar até que o avaliador apresente-se. Pessoas da equipe de STAFF circularão entre os pôsteres auxiliando caso haja necessidade.
	</p>
	<br />
	<h2>Colação de Pôster</h2>
	<p align="justify">
	<p>
		<strong>Sessão 1</strong>
	</p>
	<p>
		Para os projetos da Sessão 1 (06/10 - 10h30 às 12h30), os pôsteres deverão ser fixados no dia 06 a partir da 07h30 em local identificado com o código e nome do aluno respectivo.
	</p>
	<p>
		A retirada do pôster deverá ser feita no dia logo após o encerramento da sessão. A Coordenação não se responsabilizará por eles.
	</p>
	<p align="justify">
		Para a Sessão 1 os alunos deverão seguir os seguintes códigos:
		<ul>
			<li>
				<strong>Ciências Agrárias:</strong> ZOO, AGRO, FLORE, ZOOTC, MEDVET e CTA;
			</li>
			<li>
				<strong>Ciências Sociais Aplicadas:</strong> DIR, DIRpb, DIRpr, DIResp, DIRt, ADM, ECON, ARQUR, PLAN, COMUM, SS e DES.
			</li>
		</ul>
	</p>
	<br />
	<p>
		<strong>Sessão 2</strong>
	</p>
	<p>
		Para os projetos da Sessão 2 (06/10 - 15h30 às 17h30), os pôsteres deverão ser fixados no dia 06/10 a partir das 14h00.
	</p>
	<p>
		A retirada do pôster deverá ser feita no dia logo após o encerramento da sessão. A Coordenação não se responsabilizará por eles.
	</p>
	<p align="justify">
		Para a Sessão 2 os alunos deverão seguir os seguintes códigos:
		<ul>
			<li>
				<strong>Ciências Exatas:</strong> MAT, CCOMP, FIS, QUI, ECV, ELE, MEC, EQ, ESAN, EPR e EBIO;
			</li>
			<li>
				<strong>PIBITI:</strong> Todos.
			</li>
		</ul>
	</p>
	<br />
	<p>
		<strong>Sessão 3</strong>
	</p>
	<p>
		Para os projetos da Sessão 3 (07/10 - 10h30 às 12h30), os pôsteres deverão ser fixados no dia 07/10 a partir das 07h30.
	</p>
	<p>
		A retirada do pôster deverá ser feita no dia logo após o encerramento da sessão. A Coordenação não se responsabilizará por eles.
	</p>
	<p align="justify">
		Para a Sessão 3 os alunos deverão seguir os seguintes códigos:
		<ul>
			<li>
				<strong>Ciências Humanas:</strong> FILO, SOCIO, ANTRO, HIST, GEO, PSICO, EDU, CPOLI, TEO, LING, LETR e ART;
			</li>
			<li>
				<strong>PIBIC JR:</strong> todas as áreas.
			</li>
		</ul>
	</p>
	<br />
	<p>
		<strong>Sessão 4</strong>
	</p>
	<p>
		Para os projetos da Sessão 4 (07/10 - 15h30 às 17h30), os pôsteres deverão ser fixados no dia 07/10 a partir das 14h00.
	</p>
	<p>
		A retirada do pôster deverá ser feita no dia logo após o encerramento da sessão. A Coordenação não se responsabilizará por eles.
	</p>
	<p align="justify">
		Para a Sessão 4 os alunos deverão seguir os seguintes códigos:
		<ul>
			<li>
				<strong>Ciências da Saúde:</strong> BIOG, GEN, BIOTEC, BOT, ECO, FIS, BIOQ, BIOF, FAR, FARM, IMU, MICRO, MED, ODO, ENF, NUT, SC, FISIO, FISIOL e EF.
			</li>
		</ul>
	</p>
	</p>

	</p> 
	<p align="justify">
			<br />
	<p>
		<h1>Participação</h1>
	</p>
	<ul>
		<li>
			<p align="justify">
				<strong>Estudantes PIBIC, PIBITI e PIBIC JR</strong>
				<br/>
				No dia 05/10 das 11h30 às 19h30 o estudante que apresentará tanto na modalidade oral quanto pôster deverá retirar o KIT de apresentação na secretaria do evento, exceto estudantes dos campus fora de sede que deverão retirar o KIT no dia 06/10 a partir das 07h00. A utilização da camiseta é obrigatória no momento da apresentação.
			</p>
			<p>
				Para retirada do KIT o aluno deverá apresentar a carteirinha da PUCPR.
			</p>
			<p>
				Todos os alunos PIBIC/PIBITI deverão obrigatoriamente estar munido de seu crachá de estudante (PUCPR) durante o evento, somente com ele o aluno terá acesso aos blocos de apresentação do mesmo. A equipe de STAFF ficará na porta de cada sala de apresentação para validar a entrada e saída das salas.
			</p>
		</li>
	</ul>
	<ul>
		<li>
			<p align="justify">
				<strong>Alunos ouvintes</strong>
				<br/>
				Os alunos ouvintes que queiram participar do evento deverão estar munidos de seu crachá da PUCPR para validar a entrada e saída das salas.  O sistema contabilizará a entrada e saída apenas se o aluno permanecer no bloco inteiro de apresentação. A declaração somente será emitida se o aluno participar 50% do evento, ou seja, 7 blocos INTEIROS de apresentação e/ou palestras e mesa redonda.
			</p>
			<!--
			<p>
				Alunos de outras instituições deverão realizar o pré-credenciamento no site do <a href="http://www.pucpr.br/semic" target="_blank"><?php echo msg('link_semic');?> </a>  e emitir o código ID que será gerado, este será o código que validará a participação do aluno no evento.
			</p>
			-->
		</li>
	</ul>
		
	</p>
</div>
<br />
	<hr size="5" width="100%" align="left" noshade>
	<hr size="8" width="100%" align="left" noshade>
</div>
<div id="toTop" style="display: none;">
	^ <?php echo msg('back');?>o
</div>