...
$urlAvalia = $av->build_url_pagina($essaPagina,array("idAvaliador" => $idAvaliador, "idTrabalhoStr" => $idTrabalhoStr, "tipoTrabalho" => $tipoTrabalho));
$urlSelecionaTrabalho = $av->build_url_pagina("seleciona_trabalho", array("idAvaliador" => $idAvaliador));

define("ALUNO_NAO_COMPARECEU_AVISO", 1);
define("ALUNO_NAO_COMPARECEU_CONFIRMA", 2);

$argsNaoCompareceu = $args;
$argsNaoCompareceu['naoCompareceu'] = ALUNO_NAO_COMPARECEU_AVISO;
$urlNaoCompareceu = $av->build_url_pagina($essaPagina, $argsNaoCompareceu);
$argsNaoCompareceu['naoCompareceu'] = ALUNO_NAO_COMPARECEU_CONFIRMA;
$urlNaoCompareceuConfirma = $av->build_url_pagina($essaPagina, $argsNaoCompareceu);
...
if($erroValidaIdTrabalho == ""){
	...
	if($acaoPagina == 'avaliar' || $acaoPagina == 'trabalhoJaAvaliado'){
		...
		if($naoCompareceu == ALUNO_NAO_COMPARECEU_AVISO){
			$acaoPagina = "confirmarFaltaAluno";
		}

		elseif($naoCompareceu == ALUNO_NAO_COMPARECEU_CONFIRMA){
...
elseif($av->valida_id_trabalho($idAvaliador, $idTrabalhoStr, NULL, False) === ''){
	...
	foreach($av->get_tipos_trabalho_possiveis($idTrabalhoStr) as $tipoTrab){
...
<div class='destaque' style='color: red;'>$idTrabalhoStr</div>
...
elseif($acaoPagina == "confirmarFaltaAluno"){
	$quebraInfo = $hd->quebra("icon-question-sign");
	echo "
$quebraInfo
<div class='aviso'>
<p> <h1>ATENÇÃO</h1>
<p> Ao confirmar a falta do aluno, o trabalho será automaticamente reprovado.
<p> Verifique se o código abaixo está correto.

<div class='destaque'>$idTrabalhoStr</div>

<p> Para confirmar o não comparecimento do aluno, <i>clique no botão abaixo</i> Caso contrário, <i><a href=$urlAvalia>clique aqui</a></i>
	para retornar a página de avaliação.

<br>
<br>

<a href=$urlNaoCompareceuConfirma class='botao_acao_alternativa'> Esse aluno não compareceu</a>
