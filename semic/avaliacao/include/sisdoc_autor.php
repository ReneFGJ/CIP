...
function recupera_email($s)
	{
		$x = strpos($s,'@');
		if ($x > 0)
			{
				while (($x > 0) and (substr($s,$x,1) != ' '))
					{ $x--;	}
				$email = substr($s,$x,200);
				$email = trim(troca($email,';',''));
				return($email);
			} else {
				return('');
			}
...
function ext_autor($autor)
		...
		while (strpos($autor,chr(13)) > 0)
			...
			for ($qk=0;$qk < count($aut);$qk++)
				...
				if (strpos($aut[$qk],';') > 0)
					{
					$quali = substr($aut[$qk],strpos($aut[$qk],';'),strlen($aut[$qk]));
					}
