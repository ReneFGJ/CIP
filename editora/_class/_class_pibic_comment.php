...
class comment
	{
		...
		var $comment;
		var $avaliation;
		var $tabela = 'pibic_comment';
				
		function comment_save()
			{
				global $user_login,$nw,$dd;
				...
				$user = strzero($nw->user_codigo,7);

				$sql = "select * from ".$this->tabela;
				$sql .= " where cepc_codigo='$codigo' and 
					cepc_user='$user_login' and cepc_comment = '$comment' ";
				$rlt = db_query($sql);
				if (!($line = db_read($rlt)))
					{
...
//					from ".$this->tabela." where cepc_codigo = '$codigo' 
				...
				//while ($line = db_read($rlt))
					...
					//}					
				//$sql = "update ".$this->update." set cep_comment_pos=$p1, cep_comment_neg=$p2 where cep_codigo = '$codigo' ";
				//$rlt = db_query($sql);
		...
		function comment_form()
			{
				...
				$sx .= '<form method="post" action="'.page().'?dd0='.$dd[0].'&dd90='.$dd[90].'">';
				...
				while ($line = db_read($rlt))
					{
						...
						//$sx .= '<font class="lt0">&nbsp;'.stodbr($line['cepc_data']).' '.$line['cepc_hora'];
