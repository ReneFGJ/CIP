	...
	function dtd_autores()
		{
			...
			for ($r=0;$r < count($aut);$r++)
				...
				$corresp = 'yes';
				if ($r > 0) { $corresp = 'no'; }	
				$sx .= '<contrib contrib-type="author" corresp="'.$corresp.'">'.chr(13).chr(10);
	   			$sx .= '<name><surname>'.trim($surname).'</surname>';
						
  				if (strlen($name) > 0) { $sx .= '<given-names>'.$name.'</given-names>'.chr(13).chr(10); }
  				if (strlen($prefixo) > 0) { $sx .= '<prefix>'.$prefixo.'</prefix>'.chr(13).chr(10); }
  				if (strlen($sufixo) > 0) { 	$sx .= '<suffix>'.$sufixo.'</suffix>'.chr(13).chr(10); }
				$sx .= '
					</name>
    				<xref ref-type="aff" rid="aff'.($r+1).'">['.($r+1).']</xref>			
				</contrib>
				';	
				$aff .= '<aff id="aff'.($r+1).'"><label>'.($r+1).'</label>'.$cr;
				if (strlen($orgdiv2) > 0)
					{ $aff .= '<institution content-type="orgdiv2">'.$departamento.'</institution>'; }
				if (strlen($instituicao) > 0)
					{ $aff .= '<institution content-type="orgname">'.$instituicao.'</institution>'.$cr; }
				$aff .= '<addr-line>'.$cr;
				$aff .= '<named-content content-type="city">'.$cidade.'</named-content>'.$cr;
				$aff .= '<named-content content-type="state">'.$estado.'</named-content>'.$cr;
				$aff .= '</addr-line>'.$cr;
				$aff .= '<country>'.$pais.'</country>'.$cr;
				$aff .= '<institution content-type="original">'.$biografia.''.$cr;
				$aff .= '<named-content content-type="email">'.$email.'</named-content>'.$cr;
				$aff .= '</institution>'.$cr;
				$aff .= '</aff>'.$cr;
				}
			$sx .= '</contrib-group>';
	...
	function dtd_article_meta()
		{
			...
			if (strlen(trim($this->subtitulo)) > 0)
				{
        			$sx .= '<subtitle>'.$this->subtitulo.'</subtitle>'.chr(13).chr(10);
        		}
			if (strlen(trim($this->titulo_alt)) > 0)
				{
				$sx .= '<trans-title-group xml:lang="'.$this->idioma_alt.'">'.chr(13).chr(10);
				$sx .= '<trans-title>'.$this->titulo_alt.'</trans-title>'.chr(13).chr(10);
				$sx .= '</trans-title-group>'.chr(13).chr(10);
				}
			$sx .= '</title-group>';			
	...
	function dtd_abstract()
		{
			...
			$sx .= '<kwd-group xml:lang="'.$this->idioma.'">';
			$sx .= $this->dtd_keywords($this->keyword);
			$sx .= '</kwd-group>';
				
			$sx .= '<trans-kwd-group xml:lang="'.$this->idioma_alt.'">';
			$sx .= $this->dtd_keywords($this->keyword_alt);
			$sx .= '</trans-kwd-group>';
			return($sx);
		}
	...
	function dtd_body()
		{
			...
			for ($r=0;$r < 10;$r++)
				{
					$file = $this->filename;
					$ref = '<table no='.$r.'></table>';
					if (strpos($body,$ref) > 0)
						{
							$this->count_table = $this->count_table + 1;
							$nref = '<table-wrap id="t'.strzero($r,2).'">';
							//$nref .= '<graphic mimetype="image" xlink:href="'.$file.'_'.strzero($r,2).'.tif"/>';
							$nref .= '<graphic mimetype="image" xlink:href="'.strzero($r,2).'.tif"/>';
							$nref .= '</table-wrap>';
							$body = troca($body,$ref,$nref);		
						}
			...
			for ($r=0;$r < 10;$r++)
				{
					$file = $this->filename;
					$ref = '<figure no='.$r.'></figure>';
					//$ref = '<table no='.$r.'></table>';
					if (strpos($body,$ref) > 0)
						{
							$this->count_fig = $this->count_fig + 1;
							$nref = '<fig id="f'.strzero($r,2).'">';
							//$nref .= '<graphic mimetype="image" xlink:href="'.$file.'_'.strzero($r,2).'.tif"/>';
							$nref .= '<graphic mimetype="image" xlink:href="'.strzero($r,2).'.tif"/>';
							$nref .= '</fig>';
							$body = troca($body,$ref,$nref);		
	...
	function dtd_ref_book($ref)
		{
			...
			if ($tipo_autor=='A')
				...
				for ($r=0;$r < count($autores);$r++) {
					$sx .= '<name>'.$cr.'<surname>'.trim($autores[$r][0]).'</surname>';
					$sx .= '<given-names>'.$autores[$r][1].'</given-names>'.$cr;
               		$sx .= '</name>'.$cr;
					}
				$sx .= '</person-group>';
				} else {
			...
			$sx .= '<source xml:lang="'.$idioma.'" >'.trim($titulo).'</source>'.$cr;
          	$sx .= '<edition>'.trim($edicao).'</edition>'.$cr;
          	$sx .= '<publisher-loc>'.trim($local).'</publisher-loc>'.$cr;
          	$sx .= '<publisher-name>'.trim($journal).'</publisher-name>'.$cr;
         	$sx .= '<year>'.$ano.'</year>'.$cr;
	    	$sx .= '<size units="page">'.$page.'</size>'.$cr;
    		$sx .= '</element-citation>'.$cr;
			$sx .= '</ref>'.$cr;			
			return($sx);
		}
	...
	function dtd_ref_journal($ref=array())
		{
			...
			if (round($no) == 0)
				{
					echo 'ERRO (201) - Sem N˙mero<HR>'.$rf.'<HR>';
					echo $nref;
					exit;
				}
			...
			if ($tipo_autor=='A')
				...
				for ($r=0;$r < count($autores);$r++) {
					$sx .= '
               				<name>
                      		<surname>'.trim($autores[$r][0]).'</surname>
	                      		<given-names>'.trim($autores[$r][1]).'</given-names>
               				</name>'.chr(13).chr(10);
					}
				$sx .= '</person-group>';
				} else {
	...
	function dtd_journal_meta()
		{
			...
			<journal-meta>
				...
				'.$eissn.'
				<publisher>
       				<publisher-name>'.trim($this->publisher).'</publisher-name>
				</publisher>				
			</journal-meta>
			';
			return($sx);
