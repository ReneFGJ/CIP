...
class issue {
	...
	function cp() {
		...
		$cp = array();
		$nc = $nucleo . ":" . $nucleo;
		$njid = ' where journal_id = ' . intval($jid);
		array_push($cp, array('$H4', 'id_issue', 'id_issue', False, False, ''));
		array_push($cp, array('$Q title:journal_id:select * from journals ' . $njid . ' order by upper(asc7(title))', 'journal_id', 'Journal', True, True, ''));
	...
	function publication_list($jid = 0) {
		...
		$sql = "select * from issue 
			...
			";
		$rlt = db_query($sql);
		while ($line = db_read($rlt)) { array_push($ed, $line);
		}
		return ($ed);
	}
	...
	function article_sumary_show($ln, $ab) {
		...
		if (strlen($art_pages) > 0) { $art_pages = '<td class="pags"><small>Págs&nbsp;' . $art_pages . '&nbsp;</small>';
		}
		$art_pdf = 'PDF';
