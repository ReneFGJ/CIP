...
class parecer {
	...
	function resumo_parecer() {
		...
		$sql = "select " . $cp . " from reol_parecer_enviado 
						left join pareceristas on pp_avaliador = us_codigo and us_journal_id = '$jid'
		...
		$sql .= "select " . $cp . " from " . $this -> tabela . " 
					inner join pareceristas on pp_avaliador = us_codigo and us_journal_id = '$jid'
					inner join submit_documento on pp_protocolo = doc_protocolo
					inner join journals on doc_journal_id = jnl_codigo 
					where (pp_status = 'B' or pp_status = 'C')
					and journal_id = " . round($jid);
		...
		while ($line = db_read($rlt)) {
			...
			$data_inicial = substr($dp, 0, 4) . '-' . substr($dp, 4, 2) . '-' . substr($dp, 6, 2);
			...
			switch ($sta) {
				case 'B' :
					...
					$sa .= '<TD class="tabela01" align="center">';
					$slink = '<span class="link" onclick="newxy2(\'parecer_declaracao_liberar.php?dd0=' . $line['id_pp'] . '&dd1=' . $this -> tabela . '&dd2=' . $jid . '&dd90=' . checkpost($line['id_pp'] . $this -> tabela) . '\',300,300);" style="cursor: pointer;">';
					$sa .= $slink . $livre . '</span>';
					break;
				case 'C' :
					$sa .= '<TD class="tabela01" align="center">';
	...
	function show_status($c) {
		switch ($c) {
			case '@' :
				...
				break;
			case 'A' :
				$sx = '<font color="#FF80FF">Em análise</font>';
				break;
			case 'B' :
				$sx = '<font color="#8080FF">Avaliado</font>';
				break;
			case 'I' :
				$sx = '<font color="green">Indicado</font>';
				break;
			case 'C' :
				$sx = '<font color="#8080FF">Avaliado</font>';
				break;
			case 'D' :
				$sx = '<font color="#C0C000">Declinado</font>';
				break;
			case 'X' :
				$sx = '<font color="#808080">Cancelado</font>';
				break;
			default :
				$sx = '[' . $c . ']';
		}
	...
	function indicar_avaliadores($area = '', $journal = '') {
		...
		while ($line = db_read($rlt)) {
			$area = $line['a_cnpq'];
			if ($area != $xarea) {
				if (strlen($gra) > 0) { array_push($grs, $gra);
				}
		...
		if ($line = db_read($rlt)) {
			for ($r = 0; $r < count($cp); $r++) {
				...
				if ($fld == 'pp_p01') {
					...
					if ((strlen($fld) > 0) and (strlen($cnt) > 0)) { $cpa = trim($cp[$r][2]);
					}
				}
				if (strlen($cnt) > 0) {
					$sa .= '<BR><BR><B>';
	...
	function resumo_avaliacao_leitura() {
		...
		while ($line = db_read($rlt)) {
			...
			if ($line['pp_data_leitura'] < 20100101) { $data = 'não lido';
			}
			$sx .= '<TR>';
			$sx .= '<TD>';
			$sx .= $status[$line['pp_status']];
	...
	function mostra_mini($line) {
		...
		if ($lido > 20100101) { $lido = '<BR>	<BR><i>(lido)</I>';
		} else { $lido = '<BR><BR><B>não lido</B>';
		}
