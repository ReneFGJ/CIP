...
class parecerista {
	...
	function lista_avaliadores() {
		...
		$sx .= '<TR bgcolor="#C0C0C0"><TH>Nome<TH>Instituição<TH>Área do conhecimento<TH>Nível bolsa PQ do CNPq';
		$tot = 0;
		...
		while ($line = db_read($rlt)) {
			...
			if ($pp != $xpp) {
				if (strlen($area) > 0) {
					...
					if (strlen($email1) > 0) { enviaremail($email1, '', $title, $txt);
						echo $email1;
					}
					if (strlen($email2) > 0) { enviaremail($email2, '', $title, $txt);
						echo ', ' . $email2;
					}
					$area = '';
				}
		...
		if ($tipo == 'E') {
			...
			while ($line = db_read($rlt)) {
				...
				if (!(strpos('   ' . $ins, 'PUCPR') > 0)) {
					...
					if (strlen(trim($line['inst_abreviatura'])) > 0) { $nome .= ' (' . trim($line['inst_abreviatura']) . ')';
					}
	...
	function pareceristas_lista($tipo) {
		...
		while ($line = db_read($rlt)) {
			if (strlen(trim($line['us_codigo'])) != 8) {
				...
				if ((strlen(trim($line['us_codigo'])) == 7) and (trim($line['inst_abreviatura']) == 'PUCPR')) { $cor = '<font color="red">';
				}
				$id++;
				$ln = $line;
				$sx .= '<TR>';
	...
	function areas_novas() {
		...
		while ($line = db_read($rlt)) {
			...
			if (substr($line['a_cnpq'], 5, 2) != '00') { $id = 2;
				$sln = '</I></font>';
				$sl = '<font class="lt2"><I>';
				$style = 'style="padding: 0px 0px 0px 20px;" ';
			}
			if (substr($line['a_cnpq'], 5, 2) == '00') { $id = 1;
				$sln = '</I></font>';
				$sl = '<A name="' . substr($line['a_cnpq'], 0, 4) . '"><font class="lt2"><I>';
				$style = 'style="padding: 0px 0px 0px 20px;" ';
			}
			if (substr($line['a_cnpq'], 2, 2) == '00') { $id = 1;
				$sln = '</B></font>';
				$sl = '<font class="lt2"><B>';
				$sx .= '<TR><TD>';
				$style = '';
			...
			if ((strlen($sl) > 0) and ($id == 1)) {
				...
				if ($ativo > 0) { $sla = '<span style="area_ativa">';
					$slb = '</span>';
				}
				if (substr($line['a_cnpq'], 8, 2) == '00') { $sla .= '';
					$slb = '' . $slb;
				}

				if (strlen($sy) == 0) { $sy .= '<div id="nm' . $idc . 'e" style="display: none; padding: 0px 0px 0px 30px;">' . chr(13);
				} else { $sy .= '<BR>';
				}
	...
	function area_excluir($id) {
		global $dd, $secu, $edit;
		if (($dd[12] == 'DEL') and ($dd[11] == checkpost($dd[10])) and ($edit == 1)) {
	...
	function area_mostra($line) {
		...
		if (round($line['pa_update']) > 0) { $sx .= '<TD align="center" class="tabela00">';
		}
		...
		while ($line = db_read($rlt)) {
			...
			switch ($status) {
				case 'I' :
					...
					$status = '<font color="green">' . $linka . 'Declinar</A></font>';
					$tot2++;
					break;
				case 'C' :
					$font = '<font color="grey">';
					$status = '<font color="brown">Declinado</font>';
					$tot3++;
					break;
				case 'D' :
					$font = '<font color="grey">';
				...
				case 'B' :
					...
					$status = '<font color="brown">Finalizado</font>';
					$tot1++;
					break;
				case 'J' :
					$font = '<font color="blue">';
				...
				case 'X' :
					$font = '<font color="grey">';
					$status = '<font color="grey">Cancelado</font>';
					break;
			}
	...
	function mostra_dados() {
		...
		//$sql = "update ".$this->tabela." set us_instituicao = '0000012' where us_instituicao = '0000455'";
		//$RRR = db_query($sql);
		//echo $sql;
	...
	function status() {
		$ar = array('1' => 'Convite temporário aceito (validar)', '2' => 'Aguardando resposta', '3' => 'Recusado temporariamente', '9' => 'Enviar convite', '-1' => 'Excluir', '0' => 'Não, convite não aceito', '19' => 'Convite novo enviado', '10' => 'SIM, Convite Novo Aceito', '11' => 'Convite aceito via CNPq (temporário)');
		return ($ar);
	}
