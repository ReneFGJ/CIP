...
class artigos {
	...
	function mostra() {
		...
		if ($line['article_dt_envio'] > 20000101) { $subm = stodbr($line['article_dt_envio']);
		} else { $subm = '(não definida)';
		}
		if ($line['article_dt_aceite'] > 20000101) { $acei = stodbr($line['article_dt_aceite']);
		} else { $acei = '(não definida)';
		}
