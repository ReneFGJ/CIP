...
class submit {
	...
	function lista_submissoes_instituicoes_email() {
		global $jid;
		$wh2 = " (doc_status = '*' or doc_status = '@') ";
		//$wh = " doc_status <> 'X' ";
		$wh = " (doc_status = 'A' or doc_status = 'B' or doc_status = 'G' or doc_stauts = 'Z') ";
	...
	function lista_submissoes_instituicoes_nao_finalizada() {
		...
		while ($line = db_read($rlt)) {
			...
			switch ($sta) {
				case '@' :
					$sx .= '<font color="red">Em submissão';
					break;
				case 'A' :
					$sx .= '<font color="green">Submetido';
					break;
				case 'B' :
					$sx .= '<font color="blue">Em análise';
					break;
				case 'X' :
					$sx .= '<font color="red">Cancelado';
					break;
				default :
					$sx .= '[' . $sta . ']';
			}
	...
	function comunicar_avaliador() {
		...
		if ($line = db_read($rlt)) {
			...
			if (strlen($avaliador_email_1) > 0) { enviaremail($avaliador_email_1, '', '[REOL] ' . $titulo_email . ' - ' . $protocolo, $texto_email);
			}
			if (strlen($avaliador_email_2) > 0) { enviaremail($avaliador_email_2, '', '[REOL] ' . $titulo_email . ' - ' . $protocolo, $texto_email);
			}
	...
	function msg($ac) {
		switch($ac) {
			case 'action_A' :
				$ac = 'Indicar parecerista';
				break;
			case 'action_B' :
				$ac = 'Aceitar para avaliação';
				break;
			case 'action_@' :
				$ac = 'Devolver para ressubmissão';
				break;
			case 'action_C' :
				$ac = 'Indicar novo parecerista';
				break;
			case 'action_X' :
				$ac = 'Cancelar submissão';
				break;
			case 'action_L' :
				$ac = 'Encaminhar correções (autor)';
				break;
			case 'action_G' :
				$ac = 'Aprovar, enviar para editoração';
				break;
			case 'action_N' :
				$ac = 'Não aprovar trabalho';
				break;
			case 'action_E' :
				$ac = 'Enviar para o editor';
				break;
		}
		return ($ac);
	}
	...
	function transfere_submissao_para_works($prj_nr, $issue, $secao) {
		...
		if ($line = db_read($rlt)) {
			...
			$resumo1 = $line['doc_resumo'];
			$key1 = $line['doc_palavra_chave'];
		}
		...
		if ($line = db_read($rlt)) {
			...
			$rlt = db_query($sql);
			if ($line = db_read($rlt)) {
				$this -> protocolo_novo = $line['doc_protocolo'];
			}
		...
		/* Recupera novo protocolo */
		$sql = "select * from reol_submit where doc_protocolo_original = '" . $prj_nr . "'";
		$rlt = db_query($sql);
	...
	function show_list_publicado($st = '', $data = 0, $datai = 0, $filtro = '') {
		...
		$sql = "select * from articles
					where ASC7(article_title) LIKE '%".UpperCaseSql($st)."%'
					 limit 1";
		$rlt = db_query($sql);
		echo $sql;
		$id = 0;
	...
	function mostra_submissoes() {
		...
		$ln = $line;
		$di = $line['doc_update'];
		if (round($di) == 0) { $di = $line['doc_data'];
		}
	...
	function cp() {
		...
		array_push($cp,array('$T80:6','doc_titulo','Título',True,True));
	...
	function status() {
		$st = array('@' => 'Em submissão pelo autor', '#' => 'Em Correção de submissão pelo autor', 'A' => 'Submetido, aguardando aceite para avaliação', 'B' => 'Aceito para avaliação, aguardando indicação de parecerista', 'C' => 'Em avaliação por parecerista Ad Hoc', 'D' => 'Avaliação finalizada pelo parecerista', 'G' => 'Em análise do editor', 'O' => 'Reavaliação do parecerista Ad Hoc', 'H' => 'Aceito para publicação', 'I' => 'Aceito para publicação, falta definir edição', 'M' => 'Devolvido pelo autor com as correções', 'N' => 'Não aprovado', 'L' => 'Enviado para o autor para correções', 'Q' => 'Enviado para editora', 'P' => 'Outros', 'Z' => 'Aceito para publicação', 'X' => 'Cancelado');
		return ($st);
	}
	...
	function mostra_dados() {
		...
		if (strlen($$this -> area) > 0) { $sx .= '<TD class="lt0" align="right">área</TD><TD class="lt1"><B>' . $this -> area . '</B></TD>';
		}
		$sx .= '</TR>' . chr(13);
		$sx .= '</TR>' . chr(13);
		$sx .= '</table>' . chr(13);
	...
	function mail_resumo() {
		...
		$bgcor_01 = '';
		if ($tot_nao_lida > 0) { $bgcor_01 = ' bgcolor="#FFA0A0" ';
		}
	...
	function mostra_autores_todos() {
		...
		while ($line2 = db_read($rlt2)) {
			...
			if (strlen($lattes) > 0) { $lattes = '<A HREF="' . $lattes . '" target="new_"><img src="img/icone_lattes.gif" width="20" height="20" alt="" border="0"></A>';
			}
