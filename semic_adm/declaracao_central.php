...
if ((strlen($dd[1]) > 7) and ($acao = $bb1))
	{
	require('../_class/_class_alunopucpr.php');
...
if ($aluno->line['pa_blacklist']=='1')
	{
		echo '<h1>Aluno em Blacklist</h1>';
		echo mst($aluno->line['pa_obs']);
		exit;		
	} 
$opc = array();
...
if (strlen($cod) > 0)
	...
	while ($line = db_read($rlt))
		...
		// pb_ano
		// pb_tipo
		array_push($opc,array('Participação no programa '.trim($line['pbt_edital']).' - '.$line['pb_ano'].'/'.($line['pb_ano']+1),'O',($line['pb_ano']+1),$line['id_pb'],$dd[1]));
		}
	} else {
		$opc = array();
	}
...
for ($ro=0;$ro < count($opc);$ro++)
	{
	$link = '<A HREF="#" onclick="newxy2('.chr(39).'declaracao_emissao.php?dd10='.substr($opc[$ro][0],0,2).'&dd1='.$opc[$ro][1].'&dd2='.$opc[$ro][2].'&dd3='.$opc[$ro][3].'&dd0='.$dd[1].chr(39).',800,600);">'; 
	echo '<LI>';
	echo $link;
	echo $opc[$ro][0];
	echo '</A>';
