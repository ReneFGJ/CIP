...
while ($line = db_read($rlt))
{
	...
	$linka = '<A class="lt0" HREF="rpc_gestor_aprovacao_decisao.php?dd0='.$line['pb_protocolo'].'&dd1=APR&dd2='.checkpost(trim($line['pb_protocolo']).'APR').'">';
	$linkb = '<A class="lt0" HREF="rpc_gestor_aprovacao_decisao.php?dd0='.$line['pb_protocolo'].'&dd1=NPR&dd2='.checkpost(trim($line['pb_protocolo']).'NPR').'">';
	
	$sx .= $linka.'APROVADO'.'</a> | '.$linkb.'NÃO APROVADO'.'</A>';
	$to++;
	$ln = $line;
}
