...
$cod_aluno = $dd[0];
if ($cod_aluno != '00000000') 
	{ 
	$sql = "select * from pibic_bolsa ";
	$sql .= " where pb_aluno = '".$cod_aluno."' ";
	$sql .= " and pp_ano = '".date("Y")."' ";
	$sql .= " and pb_ativo = 1 ";
	$rlt = db_query($sql);
	if ($line = db_read($rlt))
		{ 
		$ok = -1; $msg .= 'Aluno já possui uma bolsa em ['.$line['pb_tipo'].'], ou o professor já tem esse tipo de bolsa.<BR>[C] CNPq, [F] Fundação Araucária, [P] PUCPR, [I] ICV'; 
		}	
	}
...
/////////////////////////////////////////////////////
	...
	if ($line = db_read($rlt))
	{ 
		$ok = -8; $msg .= 'Professor já tem esta bolsa!'; 
	}
//////////////////////////////////////////////////////////////// Máximo duas bolsas pagas
	if (($dd[5] == 'B') or ($dd[5] == 'G') or ($dd[5] == 'Q'))
		{
		$sql = "select count(*) as total from pibic_bolsa ";
		$sql .= " where pb_professor = '".$dd[1]."' and (pb_tipo = 'B' or pb_tipo = 'G' or pb_tipo = 'Q') and pb_ativo = 1 ";
		$sql .= " and pp_ano = '".date("Y")."' ";
		$rlt = db_query($sql);
		if ($line = db_read($rlt))
			{ 
...
////////////////////////////////////////////////////////////////
	if (($dd[5] == 'B') or ($dd[5] == 'G') or ($dd[5] == 'Q') or ($dd[5] == 'V'))
	{
	$sql = "select count(*) as total from pibic_bolsa ";
	$sql .= " where pb_professor = '".$dd[1]."' and (pb_tipo = 'B' or pb_tipo = 'G' or pb_tipo = 'Q' or pb_tipo = 'V') and pb_ativo = 1 ";
	$sql .= " and pp_ano = '".date("Y")."' ";
	$rlt = db_query($sql);
	if ($line = db_read($rlt))
		{ 
		if ($line['total'] >= ($bolsa_max_paga+$bolsa_max_icv))
			{
			$ok = -7; $msg .= 'Professor já possui '.($bolsa_max_paga+$bolsa_max_icv).' bolsas distribuídas.<BR>'; 
			}
		}
	}
