...
if ($saved > 0)
	...
	$dd2 = $dd[2];
	$dd3 = $dd[1];
	echo $pb->ativos_por_bolsa($dd1,$dd2,$dd3);
	...
	echo '<input type="submit" value="gerar arquivo de pagamento (BANCOS) >>>>" class="botao-geral">';
	echo '</form>';
	echo '</div>';
	...
	echo '<input type="hidden" name="dd1" value="'.$dd[2].'">';
	echo '<input type="hidden" name="dd2" value="'.$dd[1].'">';
	echo '<input type="submit" value="gerar arquivo de pagamento (Ordem de pagamento) >>>>" class="botao-geral">';
	echo '</form>';
	echo '</div>';	
