...
array_push($cp,array('$A8','','Resumos',False,True,''));
if (strlen($dd[3]) == 0)
	{
...
if (strlen($dd[4]) == 0) { $dd[4] = (date("Y")-1); }
	echo '<h1>Resumos não entregues</h1>';
...
if (strlen($dd[6]) > 0)
	{
		echo $pb->resumo_nao_entregue($dd[1],'',$dd[4]);
	} else {
		echo $pb->resumo_nao_entregue($dd[1],$dd[3],$dd[4]);
	}
